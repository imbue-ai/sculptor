...
API contract (`REQ-API-1`, `REQ-API-2`, `REQ-API-3`). The user cannot
tell the backend was rewritten. If an agent was mid-run when they
upgraded, it resumed and kept going (`REQ-DATA-6`).
...
are compatible and compile/run unchanged (`REQ-API-4`).
...
(`REQ-API-5`, `requirements.md` REQ-FUNC-101).
...
rewrite preserves (`REQ-PARITY-3`, `REQ-PARITY-5`).
...
today (`REQ-DEPLOY-1`, `REQ-DEPLOY-2`).
...
(`REQ-SIMP-1`, `REQ-SIMP-2`, `REQ-SIMP-3`).
...
> `REQ-*` IDs, rather than re-deriving behavior. Where this spec uses a
> `REQ-*` ID, it is this rewrite spec's own ID; where it cites
> `requirements.md`, the ID belongs to that document.

### Behavioral contract preservation (REQ-PARITY)
- **REQ-PARITY-1 (MUST):** Every behavior specified in `docs/specs/`
  ...
  re-enumerate it.
- **REQ-PARITY-2 (MUST):** All ~446 scenarios in `scenarios.md` MUST
  continue to pass, demonstrated through `scenario_coverage.md`'s mapped
  integration tests (see REQ-VERIFY).
- **REQ-PARITY-3 (MUST):** Non-functional guarantees in `requirements.md`
...
- **REQ-PARITY-4 (MUST):** External integration contracts in
...
- **REQ-PARITY-5 (MUST):** Security, privacy & telemetry guarantees in
...
- **REQ-PARITY-6 (MUST):** Platform/dependency contracts in
...
### API compatibility (REQ-API)
- **REQ-API-1 (MUST):** The new backend MUST serve the same HTTP REST
  ...
  `/api/v1/...`).
- **REQ-API-2 (MUST):** The new backend MUST serve the same WebSocket
  ...
  protocols.
- **REQ-API-3 (MUST):** The existing frontend MUST work against the new
  ...
  on them.
- **REQ-API-4 (SHOULD):** The new backend SHOULD emit an OpenAPI schema
...
- **REQ-API-5 (MUST):** The `sculpt` CLI MUST work unchanged against the
...
### Data preservation & migration (REQ-DATA)
...
- **REQ-DATA-3 (MUST):** After migration, all user-visible state the
  ...
  and correct.
- **REQ-DATA-4 (MUST):** A migrated user MUST NOT have to re-onboard or
  reconfigure (account/email, telemetry choice, project setup, etc.).
- **REQ-DATA-5 (MUST):** On-disk state MUST be preserved and remain
  ...
  database.
- **REQ-DATA-6 (MUST):** Agents that are **in-flight at upgrade time**
  ...
  architecture step.
- **REQ-DATA-7 (MUST):** The current storage *mechanism* — an
...
### Architectural simplicity (REQ-SIMP)
- **REQ-SIMP-1 (MUST):** The rewrite MUST produce the simplest
...
- **REQ-SIMP-2 (SHOULD):** The architecture SHOULD be organized so the
  frontend's actual needs drive the backend's shape, rather than the
  backend imposing concepts the frontend never models.
- **REQ-SIMP-3 (MUST):** The HTTP/WebSocket API and the frontend's
...
### Deployment & runtime (REQ-DEPLOY)
- **REQ-DEPLOY-1 (MUST):** The backend MUST continue to ship and run in
...
- **REQ-DEPLOY-2 (MUST):** The backend MUST keep serving the bundled
  frontend UI assets (the "serve static" behavior) so self-hosted
  deployments work as they do today.
- **REQ-DEPLOY-3 (MAY):** *How* the backend process is launched (the
...
### Verification (REQ-VERIFY)
- **REQ-VERIFY-1 (MUST):** The existing integration/scenario test suite
...
- **REQ-VERIFY-2 (MUST):** The test suite itself is **kept as-is in
  ...
  prior behavior.
- **REQ-VERIFY-3 (SHOULD):** Backend unit-level coverage equivalent to
  what the Python backend has SHOULD exist in the new backend, but the
  cross-stack parity guarantee rests on REQ-VERIFY-1.
...
- **Not a change to the test corpus.** Per REQ-VERIFY-2 the integration
...
- **In-flight resume mechanism (`REQ-DATA-6`):** How the new backend
