...
- **BE2 (T1) ✅ DONE (pending device confirm)** — Chat autoscroll "snaps to bottom too early". Root cause: `useAlphaAutoScroll`'s `BOTTOM_THRESHOLD` (200px) — on a short mobile viewport that's ~¼ of the screen, so pin-to-bottom re-engages well before the actual bottom. Fix: use a tighter threshold (80px) when the scroll container's `clientHeight < 700` (layout-driven, no mobile flag; desktop unchanged at 200). The 80px/700px values are first guesses — USER to confirm the snap point feels right on device (harness can't feel scroll); tune if needed.
  - REGRESSION after the main merge ✅ FIXED: the merge carried the 80px threshold only into the scroll handler's geometry sample; the state machine's other sample sites (content ResizeObserver — fires per streamed token and overwrites the handler's sample, reading-anchor restore, restoring→userControlled re-engage, streaming-start engage) still used the flat 200px, so during streaming the early snap came back at 200px. Fix: single `bottomThresholdFor(el)` helper in `scroll/geometry.ts`, used at every `geometryAtBottom` sample site (incl. `useAlphaScrollPersistence`'s at-bottom restore decision).
- **BE3 (T1) ✅ DONE (pending device confirm)** — Scroll persistence broken on mobile ("come back to a chat, not where I left it"). Two mechanisms, both REMOUNT-specific (mobile navigation unmounts the whole chat; desktop switches `taskId` while mounted, so desktop never hit them):
  1. `useAlphaVirtualizer`'s per-task height/tail caches were refs — cold on every remount, so `estimateSize` fell back to 120px/item and the restore's first apply landed screens away from the saved spot (measured headless at 390px: first apply scrollTop 11501 vs correct 32085), then visibly leapt ~4× over 2.5s as real measurements landed; any touch in that window flips authority to `userControlled`, which cancels the re-assert and strands the wrong position. Fix: module-scoped `heightCacheByTask`/`tailCacheByTask` (same LRU cap) + mount-time seeding of estimates/tail. Verified headless: the restore now lands pixel-exact (saved 25836 → restored 25836) within ~1 frame of correction.
  2. The initial restore ran in a post-paint `useEffect`, so remounts painted the pin-to-bottom position first, then jumped. Now a `useLayoutEffect` (pre-paint), matching the task-switch path.
  - Related: a save taken within the at-bottom threshold restores bottom-relative and (if streaming) re-engages `following` — with the flat 200px on mobile that ate deliberate small scroll-ups; now 80px via `bottomThresholdFor`.
  3. (found on device) A FULL PAGE RELOAD (PWA relaunch, tab eviction, dev-server reconnect) wiped the position entirely — `alphaScrollPositionAtomFamily` was memory-only, so the restore fell back to the first-visit landing (`maxScrollOffset` against cold estimates) and the reader ended a page+ above the live tail once the real tail heights landed. Fix: the atom family is now sessionStorage-backed (`atomWithStorage`, `getOnInit`, per-tab so positions can't go stale across sessions). Verified headless with `page.reload()`: scrolled-up restores via message anchor (≈50px), at-bottom re-lands at the tail and re-engages `following` while streaming.
...
- **R2-A2 (AskUserQuestion not scrollable on mobile — HARD BLOCKER, user-reported on device) — NOT FIXED.** When the agent (or the `ask_user_question` tool) poses a question, `AskUserQuestion` replaces the composer in the chat's bottom **input region** (`AlphaChatInterface.tsx:644`, inside the `[scroll area flex:1][input region]` Flex column). A tall question card (e.g. several long-description options) makes the input region taller than a short mobile viewport; the app runs at fixed viewport height with no page scroll, so the bottom of the card (later options + submit) sits **below the fold with nothing to scroll** → the user can't answer without switching to desktop. (`AskUserQuestion.module.scss` `.container`/`.card` have no `max-height`/`overflow-y`.) PROPOSED FIX (mobile, low-risk CSS): cap the question container height and let it scroll internally — e.g. on `(pointer:coarse),(width<=767px)` give `.container`/`.card` a `max-height` (relative to viewport, accounting for the keyboard/safe-area) + `overflow-y: auto`; ideally keep the footer (submit + nav dots) pinned while only the options list scrolls (the card is already a flex column — make the options region the scroll area and the `.footer` non-shrinking). Verify on device (harness can reproduce width but needs a pending question to render the card). Files: `AskUserQuestion.tsx`, `AskUserQuestion.module.scss`, possibly the input-region wrapper in `AlphaChatInterface.tsx`.
