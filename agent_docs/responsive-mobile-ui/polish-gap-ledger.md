...
- **R2-A1 (Chat text "pops in" late after layout on agent switch) — DIAGNOSED, NOT FIXED (architectural; needs user decision).** Trace: chat messages live in `taskDetailAtomFamily(taskId)` (`taskDetails.ts`), populated ONLY by the single app-level unified websocket stream (`useUnifiedStream` @ `PageLayout.tsx:79`) — there is **no** REST history fetch on open (the `/agents/{id}/messages` GET has no caller; only the send POST is used) and **no** per-task subscribe sent on navigation. The stream accumulates updates "only when something happens" (its own comment). So when you open an agent whose detail hasn't streamed yet (typically early in a session / soon after load), the chat shell paints immediately from route/layout state while the message list is still empty, then the text appears when the stream delivers that task's snapshot — a data-arrival-timing gap, not a render bug (confirmed: no `startTransition`/`Suspense`/`useDeferredValue`/loading-gate in the chat path; the virtualizer renders item content immediately — its settling only affects scroll position). Frontend mitigations to weigh: (a) **loading skeleton** while `useTaskDetail(taskId) === null` (a never-loaded task is `null`; a real empty task is non-null-empty) — clean IF idle tasks reliably receive a detail eventually; RISK = if the backend never pushes detail for an untouched idle task, a null-keyed skeleton could spin forever (a regression). (b) **Backend/stream change** — prioritize the focused task's snapshot, or add a REST history prefetch on navigation. Recommend NOT shipping (a) blind: verify new-empty-agent + untouched-idle-agent behavior on device/harness first. Files: `useUnifiedStream.ts`, `taskDetails.ts`, `useChatData.ts`, `useTaskDetail.ts`, `ChatPanelContent.tsx`.
