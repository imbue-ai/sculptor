...
- **BE1 (T1, perf) ~PARTIAL** — TipTap typing lag. Root cause confirmed by reading the code: every keystroke writes `promptDraftAtomFamily`, and `ChatInput` subscribes via `useAtom(promptDraft)` → the WHOLE `ChatInput` tree re-renders per keystroke (+ the editor's value-sync re-serializes via `getMarkdown()` a 2nd time).
  ...
  - REMAINING (the real mobile fix — DEFERRED, careful): the dominant mobile cost is `ChatInput` itself re-rendering per keystroke. Fix = **decouple the draft from `ChatInput`'s render**: (1) `useSetAtom`/`useStore` instead of `useAtom` (no per-keystroke subscription); (2) a `hasContent`/`isBypass` bailout state for the send-button disabled check; (3) `handleSend`/`handleSubmit`/`appendText`/`handleClear` read/mutate via `editorRef` + `store.get` instead of `promptDraft`; (4) the editor stays controlled only for EXTERNAL changes (clear/append/restore go imperative). HIGH care — touches shared send/draft + mention-hydration + draft-restore + bypass-commands; validate type/send/clear/append/mention/task-switch-restore. Do with hands-on testing, not blind.
