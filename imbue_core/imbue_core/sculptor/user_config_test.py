...
from imbue_core.sculptor.user_config import PiConfig
...
def test_enable_multi_harness_defaults_off() -> None:
    config = UserConfig(
        user_email="test@example.com",
        user_id="user123",
        organization_id="org123",
    ...
    assert config.enable_multi_harness is False
    assert UserConfigField["ENABLE_MULTI_HARNESS"].value == "enableMultiHarness"
