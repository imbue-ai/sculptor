...
class TestParserExceptionContainment:
    """SCU-1667 Layer 2: a parser exception on one line must not kill the turn.
    ...
    WarningAgentMessage and keep processing subsequent lines. This protects
    against the next unknown shape even after Layer 1 handles the known ones.
    """
    ...
    def test_parser_exception_becomes_warning_and_loop_continues(self) -> None:
        ...
        # raises KeyError on ``data["message"]``. This is deliberately a shape
        # Layer 1 does NOT special-case, standing in for "the next unknown shape."
