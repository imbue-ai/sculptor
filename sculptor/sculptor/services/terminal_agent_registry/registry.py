"""Declarative terminal-agent registrations (REQ-REG-1/2/3, architecture §8).
...
def load_registrations() -> list[TerminalAgentRegistration]:
    ...
    level — the no-logger-warning ratchet) and skipped — one bad file must not break the menu (REQ-REG-3). A missing
    directory yields an empty list (it is created lazily by the user).
    """
