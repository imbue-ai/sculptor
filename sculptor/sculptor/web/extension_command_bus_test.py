"""Tests for the extension command correlation bus.

The bus matches renderer replies (POSTed to the result endpoint) back to the
``sculpt extension`` request blocked on the command endpoint. These exercise the
...
def _result(correlation_id: str) -> ExtensionCommandResult:
