...
from sculptor.foundation.async_utils import AsyncCachedProperty
from sculptor.foundation.async_utils import sync_contextmanager
from sculptor.foundation.async_utils import sync_generator
...
def test_sync_generator() -> None:
    async def agen(x: int, *, y: int) -> AsyncGenerator[int, None]:
        yield x
        yield y
        yield x + y

    a, b = 1, 2
    result = list(sync_generator(agen)(a, y=b))
    assert result == [a, b, a + b]


def test_sync_generator_decorator() -> None:
    @sync_generator
    async def agen(x: int, *, y: int) -> AsyncGenerator[int, None]:
        yield x
        yield y
    ...
    a, b = 1, 2
    result = list(agen(a, y=b))
    assert result == [a, b, a + b]


def test_sync_generator_exception() -> None:
    @sync_generator
