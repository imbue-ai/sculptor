...
from sculptor.foundation.concurrency_group import AncestorConcurrentFailure
from sculptor.foundation.concurrency_group import ChildConcurrencyGroupDidNotExitError
...
from sculptor.foundation.concurrency_group import StrandTimedOutError
...
def test_concurrency_group_raises_timeout_when_not_finished_in_time() -> None:
    ...
    with pytest.raises(ConcurrencyExceptionGroup) as exception_info:
        with ConcurrencyGroup(name="outer", exit_timeout_seconds=SMALL_SLEEP) as cg:
            thread = cg.start_new_thread(target=lambda: time.sleep(LARGE_SLEEP))
...
def test_concurrency_group_does_not_raise_when_within_timeout() -> None:
    ...
    with ConcurrencyGroup(name="outer", exit_timeout_seconds=SMALL_SLEEP) as cg:
        thread = cg.start_new_thread(target=lambda: time.sleep(TINY_SLEEP))
    end_time = time.monotonic()
    assert end_time - start_time < 0.1
    assert not thread.is_alive()


@mock.patch("sculptor.foundation.thread_utils.log_exception")
@pytest.mark.filterwarnings("ignore::pytest.PytestUnhandledThreadExceptionWarning")
def test_failed_threads_raise_when_probed(mock_log_exception: mock.MagicMock) -> None:
    i = 0
    with pytest.raises(ConcurrencyExceptionGroup) as exception_info:
        with ConcurrencyGroup(name="outer") as cg:
...
@mock.patch("sculptor.foundation.thread_utils.log_exception")
@pytest.mark.filterwarnings("ignore::pytest.PytestUnhandledThreadExceptionWarning")
def test_failed_threads_raise_when_exiting(mock_log_exception: mock.MagicMock) -> None:
    i = 0
    with pytest.raises(ConcurrencyExceptionGroup) as exception_info:
        with ConcurrencyGroup(name="outer") as cg:
            thread = cg.start_new_thread(target=lambda: 1 / 0)
...
def test_do_not_allow_starting_new_strands_if_the_previous_failed(tmp_path: Path) -> None:
    ...
    with pytest.raises(ConcurrencyExceptionGroup) as exception_info:
        with ConcurrencyGroup(name="outer") as cg:
            ...
            time.sleep(SMALL_SLEEP)  # Wait for process1 to complete.
...
@mock.patch("sculptor.foundation.thread_utils.log_exception")
@pytest.mark.filterwarnings("ignore::pytest.PytestUnhandledThreadExceptionWarning")
def test_error_from_nested_group_in_another_thread_gets_properly_propagated(
...
def _create_two_nested_concurrency_groups_that_expect_parent_failure(
    ...
    with pytest.raises(ConcurrencyExceptionGroup) as exception_info:
        with concurrency_group.make_concurrency_group(name="middle") as cg_middle:
            # Signal that nested groups are created before the outer timeout
            if "nested_groups_created" in closure:
                closure["nested_groups_created"].set()
            try:
...
@mock.patch("sculptor.foundation.thread_utils.log_exception")
@pytest.mark.filterwarnings("ignore::pytest.PytestUnhandledThreadExceptionWarning")
def test_parent_failures_propagate_recursively(mock_log_exception: mock.MagicMock) -> None:
    nested_groups_created = Event()
    closure: dict[str, Any] = {"i": 0, "nested_groups_created": nested_groups_created}
    ...
    with pytest.raises(ConcurrencyExceptionGroup) as exception_info:
        with ConcurrencyGroup(name="outer", exit_timeout_seconds=0.0001) as cg_outer:
            ...
            # Wait for nested groups to be created before allowing outer group to timeout
            nested_groups_created.wait(timeout=SMALL_SLEEP)
...
def _create_nested_concurrency_group_and_run_process(
    ...
    # We expect this to fail because the process will be killed when the parent group is shut down.
    with pytest.raises(ConcurrencyExceptionGroup) as exception_info:
        with concurrency_group.make_concurrency_group(name="inner") as cg:
...
def _create_nested_concurrency_group_and_run_process_while_shutting_down(
    ...
    # We expect this to fail because the concurrency group will not be able to start new processes when shutting down.
    with pytest.raises(ConcurrencyExceptionGroup) as exception_info:
        with concurrency_group.make_concurrency_group(name="inner") as cg:
            process_started_event.set()
            time.sleep(SMALL_SLEEP)  # Wait for the parent to start shutting down if necessary.
            closure["i"] += 1
            process = cg.run_process_in_background(["sleep", str(LARGE_SLEEP)], is_checked_by_group=True)
            process.wait()
            closure["i"] += 1
...
@mock.patch("sculptor.foundation.thread_utils.log_exception")
@pytest.mark.filterwarnings("ignore::pytest.PytestUnhandledThreadExceptionWarning")
def test_new_resources_cannot_be_created_when_ancestor_has_failed_strands(mock_log_exception: mock.MagicMock) -> None:
