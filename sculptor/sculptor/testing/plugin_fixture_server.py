"""Local HTTP server that serves plugin manifests and bundles for loader tests.

The frontend loads a plugin by fetching ``<source>/manifest.json`` and then
...
the way a plugin dev server would look) with per-route control over the body,
...
class _PluginFixtureHTTPServer(http.server.ThreadingHTTPServer):
...
class _Handler(http.server.BaseHTTPRequestHandler):
    def _routes(self) -> dict[str, _Route]:
        return cast("_PluginFixtureHTTPServer", self.server).routes
...
@dataclasses.dataclass(frozen=True)
class PluginFixtureServer:
    base_url: str
    port: int
    _server: _PluginFixtureHTTPServer
    ...
    def add_plugin(
        ...
        *,
        ...
        """Register a plugin's manifest (and optional entry module); return its source URL.
        ...
        ``add_source`` -- the plugin directory URL, without ``/manifest.json``.
        """
        manifest_body = manifest if isinstance(manifest, str) else json.dumps(manifest)
        self.add_route(f"/{plugin_id}/manifest.json", body=manifest_body, content_type="application/json", cors=cors)
        if entry_js is not None:
            self.add_route(f"/{plugin_id}/{entry_name}", body=entry_js, content_type="text/javascript", cors=cors)
        return self.source_for(plugin_id)

    def source_for(self, plugin_id: str) -> str:
        return f"{self.base_url}/{plugin_id}"
...
def _start_server() -> PluginFixtureServer:
    for _attempt in range(_MAX_PORT_RETRIES):
        port = random.randint(_SAFE_PORT_MIN, _SAFE_PORT_MAX)
        try:
            server = _PluginFixtureHTTPServer(("127.0.0.1", port), _Handler)
        except OSError:
            continue
        thread = threading.Thread(target=server.serve_forever, name="plugin-fixture", daemon=True)
        thread.start()
        return PluginFixtureServer(base_url=f"http://127.0.0.1:{port}", port=port, _server=server, _thread=thread)
    raise RuntimeError(
...
@contextmanager
def spawn_plugin_fixture_server() -> Iterator[PluginFixtureServer]:
