...
export default tseslint.config(
  // Global ignores must come first
  {
    ignores: [
      ...
      "out/**",
      "storybook-static/**",
      // Built extension output sits in public/ and is not part of the host's TS
      // program. (The /extension-runtime/ stubs are generated by a Vite plugin —
      // see vite-plugins/extension-runtime-stubs.ts — not checked in here.)
      "public/extensions/**",
      // Source for runtime-loaded extensions is its own Vite project, not in
      // the host's tsconfig.
      "extensions/**",
    ],
  },
          ...
          zones: [
            ...
            },
            {
              ...
              from: "./src/pages",
                ...
                // TidyConfirmationSettingRow reads the tidy-confirmation
                // preference, which lives inside the layout snapshot state:
                // removed when workspace layout state is promoted to
                // common/state (follow-up F1).
                "./workspace/layout/atoms/savedLayout.ts",
              ],
              message: PAGE_ISOLATION_MESSAGE,
            },
            {
