...
// invalidates this extension's namespace when the key changes.
...
export const useLinearTickets = (inputs: {
  ...
  const primaryQuery = useQuery({
    queryKey: [EXTENSION_ID, "primary", branch, pullRequestUrl],
    queryFn: ({ signal }) => {
  ...
  const prLinkedQuery = useQuery({
    queryKey: [EXTENSION_ID, "prLinked", primaryId, prUrls],
  ...
  const pinnedQuery = useQuery({
    queryKey: [EXTENSION_ID, "pinned", sortedPinned],
    queryFn: async ({ signal }) => {
