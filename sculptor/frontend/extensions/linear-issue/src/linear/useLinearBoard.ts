import { useExtensionSettings, useWorkspaces, type WorkspaceView } from "@sculptor/extension-sdk";
...
// component invalidates this extension's namespace when the key changes.
...
export const useLinearBoard = (apiKey: string): LinearBoardData => {
  const query = useQuery({
    queryKey: [EXTENSION_ID, "assigned"],
  ...
  const assignments = useExtensionSettings(assignmentKeys);
