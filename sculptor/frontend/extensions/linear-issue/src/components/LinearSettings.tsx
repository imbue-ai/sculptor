...
/**
 * Extension settings: the Linear personal API key, plus the templates that seed
 * the new-workspace dialog when a workspace is created from a board ticket.
 */
export const LinearSettings = (): ReactElement => {
  const [apiKey, setApiKey] = useExtensionSetting("apiKey");
  const [titleTemplate, setTitleTemplate] = useExtensionSetting("template:title");
  const [branchTemplate, setBranchTemplate] = useExtensionSetting("template:branch");
  const [promptTemplate, setPromptTemplate] = useExtensionSetting("template:prompt");
  const queryClient = useQueryClient();

  const handleKeyChange = (value: string): void => {
    setApiKey(value);
    // Cached issues were fetched with the old key (deliberately not part of any
    // query key) — drop this extension's namespace so panels refetch with the new
    ...
    void queryClient.invalidateQueries({ queryKey: [EXTENSION_ID] });
  };
