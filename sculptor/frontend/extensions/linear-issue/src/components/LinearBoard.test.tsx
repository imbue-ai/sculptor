...
import type { NewWorkspaceModalOptions, WorkspaceView } from "@sculptor/extension-sdk";
import { useOpenNewWorkspaceModal, useExtensionSetting, useSetExtensionSetting, useWorkspaces } from "@sculptor/extension-sdk";
...
vi.mock("@sculptor/extension-sdk", () => ({
  useExtensionSetting: vi.fn(() => ["", vi.fn()]),
  useSetExtensionSetting: vi.fn(() => vi.fn()),
...
// templates), so the mocked `useExtensionSetting` serves values from a per-test
...
beforeEach(() => {
  vi.clearAllMocks();
  settings = {};
  vi.mocked(useExtensionSetting).mockImplementation((key: string) => [settings[key] ?? "", vi.fn()]);
  setBoard({});
  vi.mocked(useWorkspaces).mockReturnValue([]);
  vi.mocked(useOpenNewWorkspaceModal).mockReturnValue(vi.fn());
  vi.mocked(useSetExtensionSetting).mockReturnValue(vi.fn());
});
afterEach(() => cleanup());
...
describe("LinearBoard", () => {
  ...
  it("opens the new-workspace modal pre-filled from the ticket and records the assignment on create", async () => {
    ...
    vi.mocked(useSetExtensionSetting).mockReturnValue(setSetting);
    setApiKey("k");
    setBoard({ groups: [groupWithUnworkedTicket()] });
    renderBoard();
