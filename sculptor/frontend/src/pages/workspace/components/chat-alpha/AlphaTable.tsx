...
import styles from "./AlphaTable.module.scss";
import { skipNextScrollAdjustForItem } from "./hooks/useAlphaVirtualizer.ts";

type AlphaTableProps = {
  children: ReactNode;
};
...
export const AlphaTable = memo(({ children }: AlphaTableProps): ReactElement => {
  ...
  const handleToggleWrap = useCallback((): void => {
    ...
    // set in AlphaChatInterface). One-shot, consume-on-match — no timers.
