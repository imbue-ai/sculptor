...
describe("AlphaMarkdownBlock", () => {
  it("renders plain text", () => {
    render(<AlphaMarkdownBlock content="Hello world" />);
    expect(screen.getByText("Hello world")).toBeTruthy();
  });

  it("renders bold text", () => {
    const { container } = render(<AlphaMarkdownBlock content="This is **bold** text" />);
  ...
  it("renders italic text", () => {
    const { container } = render(<AlphaMarkdownBlock content="This is *italic* text" />);
  ...
  it("renders links with target=_blank", () => {
    render(<AlphaMarkdownBlock content="Visit [Example](https://example.com)" />);
  ...
  it("renders unordered lists", () => {
    ...
    const { container } = render(<AlphaMarkdownBlock content={md} />);
  ...
  it("preserves the original marker on each item of a non-sequential ordered list", () => {
    ...
    // AlphaMarkdownBlock reads each item's original marker back out of the
  ...
  it("renders tables", () => {
    ...
    const { container } = render(<AlphaMarkdownBlock content={markdown} />);
  ...
  it("renders inline code", () => {
    const { container } = render(<AlphaMarkdownBlock content="Use `console.log` for debugging" />);
  ...
  it("suppresses images", () => {
    const { container } = render(<AlphaMarkdownBlock content="![alt text](https://example.com/image.png)" />);
    expect(container.querySelector("img")).toBeNull();
  });

  it("renders blockquotes", () => {
    const { container } = render(<AlphaMarkdownBlock content="> This is a quote" />);
  ...
  it("handles empty content", () => {
    const { container } = render(<AlphaMarkdownBlock content="" />);
  ...
  it("renders strikethrough text", () => {
    const { container } = render(<AlphaMarkdownBlock content="This is ~~deleted~~ text" />);
  ...
  it("renders combined bold and italic", () => {
    const { container } = render(<AlphaMarkdownBlock content="***bold and italic***" />);
    const strong = container.querySelector("strong");
    const em = container.querySelector("em");
    expect(strong).toBeTruthy();
    expect(em).toBeTruthy();
  ...
  it("renders inline code with inlineCode styling, not as a code block", () => {
    const { container } = render(<AlphaMarkdownBlock content="Use `foo` here" />);
    // Should NOT produce a <pre> (that would mean AlphaCodeBlock was used)
  ...
  it("highlights search matches in plain text without error", () => {
    const { container } = render(
      <AlphaMarkdownBlock content="hello world" searchQuery="hello" activeOccurrenceIndex={0} />,
    );
    const marks = container.querySelectorAll("mark");
    expect(marks).toHaveLength(1);
    expect(marks[0].textContent).toBe("hello");
    expect(marks[0].className).toBe("alphaSearchActive");
  });

  it("highlights search matches inside bold markdown without error", () => {
    const { container } = render(
      <AlphaMarkdownBlock content="This is **bold hello** text" searchQuery="hello" activeOccurrenceIndex={0} />,
    );
    const marks = container.querySelectorAll("mark");
    expect(marks).toHaveLength(1);
    expect(marks[0].textContent).toBe("hello");
  ...
  it("highlights search matches across multiple paragraphs", () => {
    ...
    const { container } = render(<AlphaMarkdownBlock content={md} searchQuery="hello" activeOccurrenceIndex={1} />);
    const marks = container.querySelectorAll("mark");
  ...
  it("applies active class to the match at activeOccurrenceIndex", () => {
    ...
    const marks = container.querySelectorAll("mark");
    expect(marks).toHaveLength(2);
    expect(marks[0].className).toBe("alphaSearchMatch");
    expect(marks[1].className).toBe("alphaSearchActive");
  });

  it("highlights search matches inside fenced code blocks", () => {
    const md = "```\nhello world\n```";
    const { container } = render(<AlphaMarkdownBlock content={md} searchQuery="hello" activeOccurrenceIndex={0} />);
    const marks = container.querySelectorAll("mark");
    expect(marks).toHaveLength(1);
    expect(marks[0].textContent).toBe("hello");
    expect(marks[0].className).toBe("alphaSearchActive");
  ...
  describe("clickable file paths", () => {
    ...
    it("renders inline code file path as clickable", () => {
      const { container } = render(<AlphaMarkdownBlock content="See `src/index.ts`" />);
    ...
    it("does not make non-path inline code clickable", () => {
      const { container } = render(<AlphaMarkdownBlock content="Use `const x = 1` here" />);
      expect(container.querySelector('[role="link"]')).toBeNull();
    });

    it("renders plain text file path in paragraph as clickable", () => {
      const { container } = render(<AlphaMarkdownBlock content="Edited src/foo.py successfully" />);
      const link = container.querySelector('[role="link"]');
      expect(link).toBeTruthy();
    ...
    it("stops click propagation", () => {
      ...
      const { container } = render(
        <div onClick={parentSpy} role="presentation">
          <AlphaMarkdownBlock content="See `src/index.ts`" />
        </div>,
      );
      const link = container.querySelector('[role="link"]');
      expect(link).toBeTruthy();
    ...
    it("suppresses file links when enableFileLinks is false", () => {
      const { container } = render(<AlphaMarkdownBlock content="See `src/index.ts`" enableFileLinks={false} />);
      expect(container.querySelector('[role="link"]')).toBeNull();
    });

    it("strips line number from navPath when clicking", () => {
      const { container } = render(<AlphaMarkdownBlock content="See `src/file.py:42`" />);
      const link = container.querySelector('[role="link"]');
      expect(link).toBeTruthy();
    ...
    it("suppresses file links when searchQuery is active", () => {
      const { container } = render(<AlphaMarkdownBlock content="Updated src/foo.py with changes" searchQuery="src" />);
    ...
    it("strips workspace prefix from absolute paths in inline code", () => {
      render(<AlphaMarkdownBlock content="See `/mock/workspace/code/src/index.ts`" />);
    ...
    it("does not make inline code absolute path outside workspace clickable", () => {
      const { container } = render(<AlphaMarkdownBlock content="See `/etc/config.yaml` for info" />);
      expect(container.querySelector('[role="link"]')).toBeNull();
  ...
  describe("showCursor", () => {
    ...
    it("does not inject cursor by default", () => {
      const { container } = render(<AlphaMarkdownBlock content="Hello world" />);
      expect(container.querySelector(cursorSelector)).toBeNull();
    });

    it("does not inject cursor when showCursor is false", () => {
      const { container } = render(<AlphaMarkdownBlock content="Hello world" showCursor={false} />);
      expect(container.querySelector(cursorSelector)).toBeNull();
    });

    it("injects cursor span when showCursor is true", () => {
      const { container } = render(<AlphaMarkdownBlock content="Hello world" showCursor={true} />);
    ...
    it("still renders markdown content correctly when showCursor is true", () => {
      const { container } = render(<AlphaMarkdownBlock content="**bold** text" showCursor={true} />);
    ...
    it("places cursor inside heading", () => {
      const { container } = render(<AlphaMarkdownBlock content="# Heading" showCursor={true} />);
      const cursor = container.querySelector(cursorSelector);
    ...
    it("places cursor inside last list item", () => {
      ...
      const { container } = render(<AlphaMarkdownBlock content={md} showCursor={true} />);
      const cursor = container.querySelector(cursorSelector);
    ...
    it("places cursor inside blockquote", () => {
      const { container } = render(<AlphaMarkdownBlock content="> A quote" showCursor={true} />);
      const cursor = container.querySelector(cursorSelector);
    ...
    it("places cursor after code block (not inside pre)", () => {
      ...
      const cursor = container.querySelector(cursorSelector);
      expect(cursor).toBeTruthy();
      // Code blocks are replaced by AlphaCodeBlock, so cursor is appended at the container level
    ...
    it("removes cursor when showCursor changes to false", () => {
      const { container, rerender } = render(<AlphaMarkdownBlock content="Hello" showCursor={true} />);
      expect(container.querySelector(cursorSelector)).toBeTruthy();
      rerender(<AlphaMarkdownBlock content="Hello" showCursor={false} />);
      expect(container.querySelector(cursorSelector)).toBeNull();
    });

    it("repositions cursor when content changes while showCursor is true", () => {
      const { container, rerender } = render(<AlphaMarkdownBlock content="First paragraph" showCursor={true} />);
      ...
      rerender(<AlphaMarkdownBlock content="- Item A\n- Item B" showCursor={true} />);
    ...
    it("places cursor correctly for h3 through h6 headings", () => {
      for (const [level, prefix] of [
        ...
        cleanup();
        const { container } = render(<AlphaMarkdownBlock content={`${prefix} Heading`} showCursor={true} />);
        const cursor = container.querySelector(cursorSelector);
    ...
    it("cursor does not duplicate on rapid content updates", () => {
      const { container, rerender } = render(<AlphaMarkdownBlock content="A" showCursor={true} />);
      rerender(<AlphaMarkdownBlock content="AB" showCursor={true} />);
      rerender(<AlphaMarkdownBlock content="ABC" showCursor={true} />);
      rerender(<AlphaMarkdownBlock content="ABCD" showCursor={true} />);
      const cursors = container.querySelectorAll(cursorSelector);
      expect(cursors).toHaveLength(1);
    });

    it("cursor is removed on unmount", () => {
      const { container, unmount } = render(<AlphaMarkdownBlock content="Hello" showCursor={true} />);
      expect(container.querySelector(cursorSelector)).toBeTruthy();
      unmount();
  ...
  // in the message text. AlphaMarkdownBlock must turn those wrappers back into
