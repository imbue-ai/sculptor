...
describe("ChatCodeBlock", () => {
  it("renders code content in a pre > code structure", () => {
    const { container } = render(<ChatCodeBlock content="const x = 1;" />);
  ...
  it("sets data-language attribute when language is provided", () => {
    const { container } = render(<ChatCodeBlock content="print('hi')" language="python" />);
    const pre = container.querySelector("pre");
    expect(pre!.getAttribute("data-language")).toBe("python");
  });

  it("does not set data-language when language is omitted", () => {
    const { container } = render(<ChatCodeBlock content="some code" />);
    const pre = container.querySelector("pre");
  ...
  it("preserves whitespace in code content", () => {
    ...
    const { container } = render(<ChatCodeBlock content={code} />);
    expect(container.querySelector("code")!.textContent).toBe(code);
  });

  it("applies syntax highlighting when a language is provided", async () => {
    const { container } = render(<ChatCodeBlock content='print("hello")' language="python" />);
  ...
  it("renders multi-line tokens with newline separators between lines", async () => {
    ...
    const { container } = render(<ChatCodeBlock content="line1\nline2\nline3\n" language="python" />);
  ...
  it("renders individual token spans with correct text content", async () => {
    ...
    const { container } = render(<ChatCodeBlock content="const x\n" language="python" />);
  ...
  it("falls back to plain text when content changes to unsupported language", async () => {
    ...
    const { container, rerender } = render(<ChatCodeBlock content='print("hi")\n' language="python" />);
    ...
    rerender(<ChatCodeBlock content="some code\n" language="not-a-real-language" />);
