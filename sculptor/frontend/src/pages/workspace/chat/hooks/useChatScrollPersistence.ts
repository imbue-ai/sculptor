/* eslint-disable react-hooks/immutability -- This hook imperatively restores
   ...
   immutability model — same pattern as useChatAutoScroll. */
...
/** Hard cap on the re-pin chase, so an agent whose content never stabilizes for
...
export const useChatScrollPersistence = (
  ...
  // A restore that arrived before the agent's messages did (cold agent-detail
  ...
  const applyScrollPosition = useCallback((): "anchor" | "bottom" | "none" => {
    ...
    if (!scrollPosition) {
      // First visit: land at the very end of the padded scroll range. For an
  ...
  // window at the *restored* offset instead of the outgoing agent's. Per-item
  ...
  // the first painted frame of the incoming agent is already the settled one —
  // no visible correction scroll afterwards. Declared before the agent-switch
  ...
  const restore = useCallback(() => {
    ...
    // outgoing agent's settled state visible on the container (a settled-wait
    ...
    // The agent's messages may not have arrived yet (the agent-detail atom is
    ...
    // Force the measurements memo to rebuild before reading. The agent-switch
    // wipe (useChatVirtualizer's layout effect, earlier in this commit) only
    // invalidates the size cache; the measurementsCache *field* still holds
    // the array computed during the render — the outgoing agent's geometry.
    // Rebuilding here makes the first apply resolve against the incoming
    // agent's cached estimates, and gives the sweep's partial rebuilds the
    // right baseline for every row below the mounted window.
    virtualizer.getVirtualItems();

    // First apply: resolves the anchor against the virtualizer's *estimated*
    // heights — the wipe on agent switch means the items have not re-measured
    ...
    // useChatAutoScroll's pinToBottom) — and clears it at every exit, so a
    ...
    const tick = (): void => {
      ...
      if (stableFrames >= BOTTOM_CONVERGE_STABLE_FRAMES || elapsedFrames >= BOTTOM_CONVERGE_MAX_FRAMES) {
        ...
        console.log(
          `[chat-scroll] bottom restore settled: frames=${elapsedFrames}, lastGrowth=${lastGrowthFrame}` +
  ...
  // Fire a pending restore the moment the agent's messages arrive. Pre-paint
  ...
  // for the same reason as the agent-switch restore above: on mobile every
  // navigation REMOUNTS the chat, and a post-paint restore would flash the
  // pin-to-bottom position (painted by useChatAutoScroll's mount effects)
