...
import { useChatAutoScroll } from "../useChatAutoScroll.ts";
...
describe("useChatAutoScroll", () => {
  ...
  it("starts disengaged", () => {
    ...
    const { result } = renderHook(() => useChatAutoScroll(ref, false, 10, virtualizer, null, -1, "test-agent"));
  ...
  it("reports isAtBottom as true initially (no scroll state)", () => {
    ...
    const { result } = renderHook(() => useChatAutoScroll(ref, false, 5, virtualizer, null, -1, "test-agent"));
  ...
  it("engages when at bottom and streaming starts (via effect, no scroll needed)", () => {
    ...
    const { result } = renderHook(() => useChatAutoScroll(ref, true, 10, virtualizer, null, -1, "test-agent"));
  ...
  it("disengages when streaming stops", () => {
    ...
    const { result, rerender } = renderHook(
      ({ isStreaming }) => useChatAutoScroll(ref, isStreaming, 10, virtualizer, null, -1, "test-agent"),
  ...
  it("auto-engages within the tighter short-viewport threshold", () => {
    ...
    const { result, rerender } = renderHook(
      ({ isStreaming }) => useChatAutoScroll(ref, isStreaming, 10, virtualizer, null, -1, "test-task"),
      { initialProps: { isStreaming: false } },
    );
  ...
  describe("following pin direction", () => {
    ...
    it("leaves an overshoot in place when not following (down-only preserved)", () => {
      ...
      const ref = { current: el };
      const virtualizer = createMockVirtualizer();
      const { result } = renderHook(() => useChatAutoScroll(ref, false, 10, virtualizer, null, -1, "test-task"));
  ...
  it("scrolls to bottom when messageCount increases while at bottom (pin-to-bottom)", () => {
    ...
    const { rerender } = renderHook(
      ({ messageCount }) => useChatAutoScroll(ref, false, messageCount, virtualizer, null, -1, "test-agent"),
      {
  ...
  describe("scroll-to-top on send", () => {
    ...
    it("fires on new user message", () => {
      ...
      const { result, rerender } = renderHook(
        ({ messageCount, lastMessageRole, lastUserMessageIndex }) =>
          useChatAutoScroll(ref, false, messageCount, virtualizer, lastMessageRole, lastUserMessageIndex, "test-agent"),
        {
          initialProps: {
            messageCount: 10,
    ...
    it("isAtBottom tracks via ResizeObserver even when not engaged during streaming", () => {
      ...
      const { result } = renderHook(() =>
        useChatAutoScroll(ref, true, 10, virtualizer, ChatMessageRole.USER, -1, "test-agent"),
      );
  ...
  it("scroll-to-top updates paddingEnd and content height when virtualizer padding is stale", () => {
    ...
    const { rerender } = renderHook(
      ...
      }) => useChatAutoScroll(ref, false, messageCount, virtualizer, lastRole, lastUserIdx, "test-agent"),
      {
        initialProps: {
          messageCount: 2,
          lastUserIdx: 0,
    ...
    it("resets isAtBottom, isEngaged, and isJumpSuppressed on agent switch", () => {
      ...
      const { result, rerender } = renderHook(
        ({ agentId, isStreaming, messageCount, lastUserMessageIndex }) =>
          useChatAutoScroll(ref, isStreaming, messageCount, virtualizer, null, lastUserMessageIndex, agentId),
        {
          initialProps: {
            agentId: "agent-a",
            isStreaming: true,
    ...
    it("does not trigger scroll-to-top on agent switch when lastUserMessageIndex stays the same", () => {
      ...
      const { rerender } = renderHook(
        ({ agentId, messageCount, lastUserMessageIndex }) =>
          useChatAutoScroll(ref, false, messageCount, virtualizer, ChatMessageRole.USER, lastUserMessageIndex, agentId),
        {
          initialProps: {
            agentId: "agent-a",
            messageCount: 5,
    ...
    it("does not trigger spurious pin-to-bottom on agent switch", () => {
      ...
      const { rerender } = renderHook(
        ({ agentId, messageCount }) =>
          useChatAutoScroll(ref, false, messageCount, virtualizer, ChatMessageRole.ASSISTANT, -1, agentId),
        {
          initialProps: {
            agentId: "agent-a",
            messageCount: 10,
