...
// Mirrors the ChatInterface wiring:
// useChatAutoScroll + useChatSearch + the search-suppression effect.
const useSearchSuppression = (
  ...
  const { isSuppressed, setIsSuppressed } = useChatAutoScroll(
    scrollContainerRef,
    isStreaming,
    messageCount,
    virtualizer,
  ...
  const { isSearchVisible } = useChatSearch(messages, virtualizer);
...
describe("search suppresses auto-scroll", () => {
  ...
  it("search navigation still scrolls to matches while auto-scroll is suppressed", () => {
    ...
    const { result: searchResult } = renderHook(() => useChatSearch(messages, virtualizer), {
      wrapper: createTestWrapper(store),
    });
