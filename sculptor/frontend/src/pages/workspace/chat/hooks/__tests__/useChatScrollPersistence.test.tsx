...
import { useChatScrollPersistence } from "../useChatScrollPersistence.ts";
...
describe("useChatScrollPersistence", () => {
  beforeEach(() => {
    vi.useFakeTimers();
  ...
  it("scrolls to the padded max scroll on first visit (no saved position)", () => {
    ...
    renderHook(() => useChatScrollPersistence(ref, virtualizer, "agent-1", messages, machine, { current: false }), {
  ...
  it("saves scroll position on scroll events", () => {
    ...
    const savedPosition = store.get(chatScrollPositionAtomFamily("agent-1"));
  ...
  it("restores to the saved distance from the content bottom when near the bottom", () => {
    ...
    store.set(chatScrollPositionAtomFamily("agent-1"), {
  ...
  it("keeps re-pinning a near-bottom restore as measurements grow the content after mount", () => {
    // Cold agent switch: off-screen rows start at their estimated heights, so the
  ...
  it("hands control back even when the content never stops growing", () => {
    // An agent that streams a token every frame never gives the loop two
  ...
  it("settles against swept measurements before the deferred re-assert frames", () => {
    ...
    // 400 — the way real measurements replace estimates on an agent switch.
  ...
  it("holds the restore pending until messages arrive, then applies it", () => {
    // Right after an agent switch the agent-detail atom can still be cold: the
    ...
    const { rerender } = renderHook(
      ({ messages }: { messages: Array<{ id: string }> }) =>
        useChatScrollPersistence(ref, virtualizer, "agent-1", messages, machine, { current: false }),
  ...
  it("does not overwrite the saved position while the restore is pending", () => {
    ...
    store.set(chatScrollPositionAtomFamily("agent-1"), saved);

    const machine = createScrollStateMachine();
    renderHook(() => useChatScrollPersistence(ref, virtualizer, "agent-1", [], machine, { current: false }), {
      wrapper: wrapperFor(store),
    });
    ...
    expect(store.get(chatScrollPositionAtomFamily("agent-1"))).toEqual(saved);
  });
  ...
  it("does not save positions produced by programmatic scrolls", () => {
    ...
    const machine = createScrollStateMachine();
    renderHook(
      () => useChatScrollPersistence(ref, virtualizer, "agent-1", messages, machine, isProgrammaticScrollRef),
      {
        wrapper: wrapperFor(store),
      },
    );
    ...
    expect(store.get(chatScrollPositionAtomFamily("agent-1"))).toBeNull();
    ...
    expect(store.get(chatScrollPositionAtomFamily("agent-1"))?.firstVisibleMessageId).toBe("msg-2");
  });
});
