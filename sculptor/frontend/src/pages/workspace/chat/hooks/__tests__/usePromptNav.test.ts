...
import { usePromptNav } from "../usePromptNav.ts";
...
/**
 * Build a controller matching the shape useActivePromptIndex returns,
 * suitable for driving usePromptNav in a unit test. `setIndex` is a mock
...
describe("usePromptNav", () => {
  ...
  const render = (
    ...
    result: ReturnType<
      typeof renderHook<ReturnType<typeof usePromptNav>, { msgs: ReadonlyArray<ChatMessage> }>
    >["result"];
    rerender: ReturnType<
      typeof renderHook<ReturnType<typeof usePromptNav>, { msgs: ReadonlyArray<ChatMessage> }>
    >["rerender"];
    ...
    const { result, rerender, unmount } = renderHook(
      ({ msgs }) => usePromptNav(msgs, virtualizer, mockScrollToBottom, mockSetIsSuppressed, controller),
  ...
  it("unmount cancels any pending highlight animation frame", () => {
    ...
    const { result, unmount } = renderHook(
      () => usePromptNav(buildMessages(), virtualizer, mockScrollToBottom, mockSetIsSuppressed, controller),
      {},
    );
