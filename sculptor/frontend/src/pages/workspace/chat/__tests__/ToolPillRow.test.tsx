...
import type * as ToolPopoverModule from "../ToolPopover.tsx";

vi.mock("~/pages/workspace/hooks/useWorkspaceCodePath.ts", () => ({
  useWorkspaceCodePath: (): string => "/workspace/code",
}));

vi.mock("../ToolPopover.tsx", async (importOriginal) => {
  // Keep `ToolEntryContent` real — ExpandedToolRow consumes it for the
  // per-tool inlined header content. Only override the popover surface.
  const actual = await importOriginal<typeof ToolPopoverModule>();
  return {
    ...actual,
    ToolPopover: ({ pillData }: { pillData: { label: string } }): ReactElement => (
      <div data-testid="tool-popover">{pillData.label}</div>
    ),
  };
});

vi.mock("../CommandPopover.tsx", () => ({
  CommandPopover: ({ toolName }: { toolName: string }): ReactElement => (
...
type PillRowProps = React.ComponentProps<typeof ToolPillRow>;
...
const renderPillRow = (
  ...
  return render(<ToolPillRow {...defaultProps} />, { wrapper: WrapperWithStore });
};
