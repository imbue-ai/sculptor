...
import { ExitPlanModeBlock } from "./ExitPlanModeBlock.tsx";
import { renderToolSegments } from "./renderToolSegments.tsx";
import { SubagentPill } from "./SubagentPill.tsx";
import { CompletedToolLine, ToolLine } from "./ToolLines.tsx";
...
// AskUserQuestionBlock / ExitPlanModeBlock via the
...
export const ToolBlockGroup = ({
  ...
  //   `supportsSubAgents` hides the SubagentPill
  //   `supportsInteractiveBackchannel` hides AskUserQuestionBlock + ExitPlanModeBlock
  ...
  const { subagentBlocks, topLevelBlocks, specialBlocks, regularBlocks } = useMemo(() => {
    ...
    for (const block of blocks) {
      ...
      if (isToolUseBlock(block)) {
        ...
        // their content is already rendered inside the SubagentPill.
      ...
      {canRenderInteractiveBackchannel &&
        specialBlocks.map((block) => {
          if (block.interactiveRole === "ask_user_question") {
            return <AskUserQuestionBlock key={block.id} toolBlock={block} />;
          }

          if (block.interactiveRole === "exit_plan_mode") {
            return <ExitPlanModeBlock key={block.id} toolBlock={block} />;
          }
          return null;
        })}
