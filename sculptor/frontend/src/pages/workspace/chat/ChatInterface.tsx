...
import { buildSubagentMetadataMap, buildSubagentTree } from "~/pages/workspace/chat/utils/subagentTree.ts";
...
import { MessageNode } from "./ChatView.tsx";
import { ErrorInput } from "./ErrorInput.tsx";
import { useActivePromptIndex } from "./hooks/useActivePromptIndex.ts";
import { useChatAutoScroll } from "./hooks/useChatAutoScroll.ts";
import type { ChatData } from "./hooks/useChatData.ts";
import { ChatScrollProvider } from "./hooks/useChatScroll.tsx";
import { useChatScrollPersistence } from "./hooks/useChatScrollPersistence.ts";
import { useChatSearch } from "./hooks/useChatSearch.ts";
import { useChatVirtualizer } from "./hooks/useChatVirtualizer.ts";
import { useJumpToBottom } from "./hooks/useJumpToBottom.ts";
import { usePromptNav } from "./hooks/usePromptNav.ts";
import { JumpToBottomButton } from "./JumpToBottomButton.tsx";
import { PromptNavigator } from "./PromptNavigator.tsx";
...
type ChatInterfaceProps = ChatData & {
...
export const ChatInterface = ({
  ...
  const { isAtBottom, scrollToBottom, setIsSuppressed, isJumpSuppressed, isUserScrollingRef } = useChatAutoScroll(
    scrollContainerRef,
    isStreaming,
    filteredNodes.length,
    virtualizer,
  ...
  const activePromptIndex = useActivePromptIndex(
    userPromptIndices,
    virtualizer,
    scrollContainerRef,
    isAtBottom,
  ...
  const { exitNavigation, navigateToPrompt } = usePromptNav(
    filteredChatMessages,
    virtualizer,
    scrollToBottom,
    setIsSuppressed,
  ...
  const {
    ...
    isSearchVisible,
    query: searchQuery,
  } = useChatSearch(filteredChatMessages, virtualizer);
  ...
  // Each text block in a message gets its own ChatMarkdownBlock that resets
