...
import { getModelShortName, hasNoUsableModel, PRODUCTION_MODELS } from "~/common/utils/modelConstants.ts";
...
import { CapabilityGate } from "~/pages/workspace/chat/CapabilityGate.tsx";
import { SendButton } from "~/pages/workspace/chat/SendButton.tsx";
...
export const ChatInput = ({
  ...
  const draftAtom = useMemo(() => promptDraftAtomFamily(agentId ?? ""), [agentId]);
  ...
  // Stable per-agent initial content for the editor. The editor is uncontrolled after
  ...
  // Re-sync the emit ref + bailout flags when the active agent (and its persisted
  // draft) changes; this component doesn't subscribe to the atom for renders.
  useEffect(() => {
    lastEditorEmitRef.current = draftStore.get(draftAtom);
    // eslint-disable-next-line react-hooks/set-state-in-effect -- sync flags to the new agent's persisted draft on agent switch; not derivable during render without subscribing
  ...
  const handleInterruptAndSend = useCallback(async (): Promise<void> => {
    if (isSendingRef.current) return;
    if (isMissingUsableModel) return;
    if (!getDraft()?.trim() || !agentId) return;
    await sendMessage();
    if (isAgentBusy) {
      await interruptWorkspaceAgent({ path: { workspace_id: workspaceID, agent_id: agentId } });
    }
  }, [getDraft, agentId, sendMessage, isAgentBusy, workspaceID, isMissingUsableModel]);
  ...
  //
  // The `isCancellable` gate mirrors the StatusPill's Stop button — it
                    ...
                    <DropdownMenu.Sub>
                      ...
                      <DropdownMenu.SubContent className="mobileTheme">
                        {hasBackendModelSource ? (
                          // A pi agent: the catalog is server-sourced and selection is
