...
export const SubagentPill = ({
  parentBlock,
  childNodes,
  toolResultMap,
  subagentMetadataMap,
  rowIndex,
}: SubagentPillProps): ReactElement => {
  ...
  const handleKeyDown = useCallback(
    (e: KeyboardEvent<HTMLButtonElement>): void => {
      ...
      // can handle it (matching ToolPillRow's behavior). stopPropagation
    ...
    <Popover.Root open={isOpen} onOpenChange={setOpen}>
      <div className={styles.row}>
        ...
        {/* Hover zone wraps the pill so the hit-area can extend slightly past
            the visible button — same pattern ToolPillRow uses — and so
