...
describe("useAlphaScrollPersistence", () => {
  ...
  it("scrolls to the padded max scroll on first visit (no saved position)", () => {
    ...
    renderHook(() => useAlphaScrollPersistence(ref, virtualizer, "agent-1", messages, machine, { current: false }), {
  ...
  it("holds the restore pending until messages arrive, then applies it", () => {
    ...
    const { rerender } = renderHook(
      ({ messages }: { messages: Array<{ id: string }> }) =>
        useAlphaScrollPersistence(ref, virtualizer, "agent-1", messages, machine, { current: false }),
  ...
  it("does not overwrite the saved position while the restore is pending", () => {
    ...
    store.set(alphaScrollPositionAtomFamily("agent-1"), saved);

    const machine = createScrollStateMachine();
    renderHook(() => useAlphaScrollPersistence(ref, virtualizer, "agent-1", [], machine, { current: false }), {
      wrapper: wrapperFor(store),
    });
    ...
    expect(store.get(alphaScrollPositionAtomFamily("agent-1"))).toEqual(saved);
  });
  ...
  it("does not save positions produced by programmatic scrolls", () => {
    ...
    const machine = createScrollStateMachine();
    renderHook(
      () => useAlphaScrollPersistence(ref, virtualizer, "agent-1", messages, machine, isProgrammaticScrollRef),
      {
        wrapper: wrapperFor(store),
      },
    );
    ...
    expect(store.get(alphaScrollPositionAtomFamily("agent-1"))).toBeNull();
    ...
    expect(store.get(alphaScrollPositionAtomFamily("agent-1"))?.firstVisibleMessageId).toBe("msg-2");
  });
});
