...
// reducers (utils/layoutApply.ts, utils/layoutCapture.ts) and writes it through the
...
import { applyCapturedLayout, computeTidyClosure } from "~/pages/workspace/layout/utils/layoutApply.ts";
import { captureLayout } from "~/pages/workspace/layout/utils/layoutCapture.ts";
import { isSystemLayoutId, SYSTEM_DEFAULT_LAYOUT_ID } from "~/pages/workspace/layout/utils/systemDefaultLayout.ts";
...
import { workspaceLayoutAtom } from "./section.ts";
import { closePanelAtom } from "./sectionActions.ts";
import { activeSectionRingNonceAtom, layoutTidyTargetAtom, maximizedSectionAtom } from "./transient.ts";

// Move an id to the front of the MRU list (most-recently-applied first),
// de-duplicating any earlier occurrence.
const withFront = (ids: ReadonlyArray<string>, id: string): Array<string> => {
