...
import type { SavedLayout, WorkspaceLayoutState } from "~/pages/workspace/layout/persistence/snapshot.ts";
...
import {
  ...
  tidyToLayoutAtom,
  updateLayoutAtom,
} from "./savedLayoutActions.ts";
import { activeWorkspaceIdAtom, panelsInSubSectionAtom, workspaceLayoutAtom } from "./section.ts";
import { activeSectionRingNonceAtom, layoutTidyTargetAtom, maximizedSectionAtom } from "./transient.ts";

const storeWith = (layout: Partial<WorkspaceLayoutState>, workspaceId = "ws-1"): ReturnType<typeof createStore> => {
...
const makeLayout = (id: string, overrides: Partial<SavedLayout> = {}): SavedLayout => {
