...
export const buildExtensionPanelDefinitions = (
...
// component ONLY because the agent snapshot hasn't arrived yet. Agent panels
// are registered off the agent snapshot (via useWorkspaceDynamicPanels), which
// isn't loaded until the first agent frame — so on a reload the layout still names
// an `agent:<agentId>` panel the registry can't resolve, and the sub-section would
...
//
// The loaded-vs-loading signal is `agentIdsAtom` (the ids list, `undefined` until
// the first frame), NOT `agentsArrayAtom`: the array subscribes to every agent
...
// agent is created or deleted, and `agentIdsAtom === undefined` iff
// `agentsArrayAtom === undefined`, so this is equivalent but far cheaper.
//
// It flips false the moment the agent snapshot arrives: the placed panel then either
// registers (a component resolves, so this is already false) or its agent is gone
