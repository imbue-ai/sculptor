...
import type { GlobalLayoutState, LayoutScope, SavedLayout, WorkspaceLayoutState } from "./snapshot.ts";
import { DEFAULT_SECTION_SIZES, LAYOUT_SNAPSHOT_VERSION, SAVED_LAYOUT_VERSION } from "./snapshot.ts";
...
const makeGlobalLayout = (overrides: Partial<GlobalLayoutState> = {}): GlobalLayoutState => {
  return {
    sidebarWidthPx: 300,
...
const makeSavedLayout = (id: string, name: string): SavedLayout => {
  return {
    id,
    name,
    captured: {
