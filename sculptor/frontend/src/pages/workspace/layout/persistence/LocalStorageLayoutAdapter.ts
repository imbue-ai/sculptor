...
import type { LayoutScope, LayoutSnapshotFor, SidebarOrderState } from "./snapshot.ts";
import { DEFAULT_GLOBAL_LAYOUT, EMPTY_WORKSPACE_LAYOUT, LAYOUT_SNAPSHOT_VERSION } from "./snapshot.ts";
...
const isValidSidebarOrder = (value: unknown): value is SidebarOrderState => {
  return (
    isObject(value) &&
...
const isValidSavedLayout = (value: unknown): boolean => {
  if (
    !isObject(value) ||
...
const normalizeSnapshot = <TScope extends LayoutScope>(
  scope: TScope,
