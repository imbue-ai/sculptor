...
// back to System Default (see atoms/savedLayout.ts), so there is always a valid
// default.
...
import { DEFAULT_SECTION_SIZES, SAVED_LAYOUT_VERSION } from "~/pages/workspace/layout/persistence/snapshot.ts";
...
const buildSystemDefaultLayout = (): SavedLayout => {
...
const presetLayout = (id: string, name: string, captured: Partial<CapturedLayout>): SavedLayout => {
...
export const isSystemLayoutId = (id: string): boolean => {
