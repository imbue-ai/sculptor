...
export const MobileWorkspaceShell = ({ taskID }: { taskID: string }): ReactElement => {
  ...
  // float over an unrelated chat. Adjusting state during render on a key change
  // is React's recommended reset pattern and avoids the cascading-render effect.
  const surfaceKey = `${workspaceID}:${taskID}`;
  const [prevSurfaceKey, setPrevSurfaceKey] = useState(surfaceKey);
  if (prevSurfaceKey !== surfaceKey) {
    setPrevSurfaceKey(surfaceKey);
    setOverlay(null);
