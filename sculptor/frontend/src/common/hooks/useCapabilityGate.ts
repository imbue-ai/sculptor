...
/** Copy shown when an affordance is unavailable because the active task's
...
/**
 * Resolve a narrow capability value (the result of a `useTaskSupports<X>`
 * hook) into a gate decision for one affordance. `?? true` keeps the affordance
 * enabled until the task's capabilities have loaded; a harness that reports the
 ...
 * render beside the `useTaskSupports<X>` hook it consumes.
 */
