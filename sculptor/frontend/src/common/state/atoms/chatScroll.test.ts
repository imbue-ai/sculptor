...
import type { ChatScrollPosition } from "./chatScroll";
import { chatScrollPositionAtomFamily, MAX_PERSISTED_SCROLL_POSITIONS } from "./chatScroll";

const position = (overrides?: Partial<ChatScrollPosition>): ChatScrollPosition => ({
...
describe("chatScrollPositionAtomFamily", () => {
  beforeEach(() => {
    localStorage.clear();
  });
  ...
  it("returns null when no position has been stored", () => {
    ...
    expect(store.get(chatScrollPositionAtomFamily("fresh-task"))).toBeNull();
  });
  ...
  it("persists writes to localStorage under the agent-keyed key", () => {
    ...
    store.set(chatScrollPositionAtomFamily("write-task"), saved);
  ...
  it("rehydrates a stored position on the first read after an app restart", () => {
    ...
    const store = createStore();
    expect(store.get(chatScrollPositionAtomFamily("restart-task"))).toEqual(saved);
  });
  ...
  it("keeps positions isolated per agent", () => {
    ...
    store.set(chatScrollPositionAtomFamily("task-a"), first);
    store.set(chatScrollPositionAtomFamily("task-b"), second);

    expect(store.get(chatScrollPositionAtomFamily("task-a"))).toEqual(first);
    expect(store.get(chatScrollPositionAtomFamily("task-b"))).toEqual(second);
  });
  ...
  it("evicts the oldest-saved positions once the cap is exceeded", () => {
    ...
    for (let i = 0; i <= MAX_PERSISTED_SCROLL_POSITIONS; i++) {
      store.set(chatScrollPositionAtomFamily(`evict-task-${i}`), position({ savedAtMs: i + 1 }));
    }
  ...
  it("evicts entries missing savedAtMs before timestamped ones", () => {
    ...
    for (let i = 0; i < MAX_PERSISTED_SCROLL_POSITIONS; i++) {
      store.set(chatScrollPositionAtomFamily(`fill-task-${i}`), position({ savedAtMs: i + 1 }));
    }
