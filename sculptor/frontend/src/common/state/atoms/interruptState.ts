...
// Per-agent "interrupt in flight" flag backing `useInterruptAgent`. The hook
...
export const isInterruptingAtomFamily = atomFamily<AgentID, PrimitiveAtom<boolean>>(() => atom<boolean>(false));

// Per-agent "Stop is clickable right now" flag. Mirrors `useAgentStatus().isCancellable`
...
export const isCancellableAtomFamily = atomFamily<AgentID, PrimitiveAtom<boolean>>(() => atom<boolean>(false));
