...
const prunePersistedPositions = (justWrittenKey: string): void => {
  ...
  for (let i = 0; i < localStorage.length; i++) {
    ...
    let savedAtMs = 0;
    try {
      savedAtMs = (JSON.parse(localStorage.getItem(key) ?? "null") as ChatScrollPosition | null)?.savedAtMs ?? 0;
    } catch {
...
// depends on.
const chatScrollStorage = createJSONStorage<ChatScrollPosition | null>(() => ({
...
export const chatScrollPositionAtomFamily = atomFamily((agentId: string) =>
  atomWithStorage<ChatScrollPosition | null>(`${STORAGE_KEY_PREFIX}${agentId}`, null, chatScrollStorage, {
    getOnInit: true,
  }),
);
