...
import { POINTER_DRAG_ACTIVATION_DISTANCE_PX } from "~/pages/workspace/layout/utils/panelDnd.ts";
...
// closestCenter.
const coveredFractionOfTarget = (dragged: ClientRect, target: ClientRect): number => {
  const overlapWidth =
...
export const useSidebarDndSensors = (): Array<SensorDescriptor<SensorOptions>> => {
  return useSensors(
