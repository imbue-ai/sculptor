...
- `~/components/useWorkspaceTabActions.ts` returns `navigateToNextTab`,
  the "this tab is going away — move to a sensible neighbour" rule. The
  sidebar's delete flow and the Cmd+K delete confirmation both consume
  this one hook.

The hook centralizes "pick the next tab AND navigate to it AND handle
pseudo-tabs (Home/Settings)" so neither caller has to reinvent half of it.
