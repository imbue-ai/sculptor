...
// All selectable layouts: System Default first, then the user's saved layouts,
// skipping any whose captured version this build can't read. The switcher's full
// candidate set before MRU ordering / filtering.
export const resolvedLayoutsAtom: Atom<ReadonlyArray<SavedLayout>> = atom((get) => {
  ...
  return [SYSTEM_DEFAULT_LAYOUT, ...saved];
});
