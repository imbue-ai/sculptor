...
// re-render on agent/layout churn — and on open it must still build the list
...
import { agentAtomFamily, agentIdsAtom, agentsArrayAtom } from "~/common/state/atoms/agents.ts";
...
const registryWithAgentProvider = (store: Store): CommandRegistry => {
  const registry = new CommandRegistry();
  registry.registerProvider({
    id: "test.tasks",
    produce: () =>
      (store.get(agentsArrayAtom) ?? []).map(
        (agent) =>
          ({
            id: `test.task.${agent.id}`,
            title: `Task ${agent.id}`,
            group: "navigation",
            perform: (): void => {},
          }) as Command,
...
describe("useVisibleCommands", () => {
  ...
  it("does not re-render the host on agent or layout churn while the palette is closed", () => {
    ...
    act(() => {
      ...
      store.set(agentAtomFamily("t1"), agentFor("t1", "ws-test"));
    });
    act(() => {
      store.set(agentAtomFamily("t1"), agentFor("t1", "ws-test", { title: "tick" }));
    });
    act(() => {
  ...
  it("builds the first-open list from state written while the palette was closed", () => {
    const store = createStore();
    const registry = registryWithAgentProvider(store);
