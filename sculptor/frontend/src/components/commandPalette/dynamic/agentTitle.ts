 ...
 */
export const agentDisplayTitle = (agent: { title?: string | null; initialPrompt: string }): string => {
  const display = agent.title?.trim() || agent.initialPrompt.trim() || "Untitled agent";
