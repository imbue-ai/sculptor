...
import { appliedLayoutIdAtom, layoutMruAtom, resolvedLayoutsAtom } from "~/pages/workspace/layout/atoms/savedLayout.ts";
 ...
 *
 * Any atom read here MUST be listed in `dynamicProviderInputsAtom`
 * (hooks/useCommandPalette.ts) so the palette recomputes when layouts / MRU /
 * the applied pointer change while open.
 */
