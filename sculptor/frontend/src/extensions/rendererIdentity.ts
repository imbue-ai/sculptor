 ...
 * the backend's "one identity per page load" contract for `ExtensionCommandResult`.
 */
const RENDERER_ID_KEY = "sculptor-extension-renderer-id";

/**
 * A stable id for this renderer instance, reused across every extension-command
...
/**
 * This renderer's self-reported identity for an `ExtensionCommandResult`. The
 ...
 * hold different extension state.
 *
 ...
 * so the base is what lets `sculpt extension list` name which bundle a window
 * is actually running.
 */
