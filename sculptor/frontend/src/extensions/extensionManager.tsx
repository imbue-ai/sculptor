...
import {
  ...
  extensionHomeViewsAtom,
  extensionOverlaysAtom,
  extensionPanelsAtom,
...
type ApiFactory = (manifest: ExtensionManifest) => ExtensionHostApi;

/**
 * The loader runs in two phases so the manager can resolve conflicts between
 * sources that provide the same extension id *before* any of them activates. A
 ...
 *   1. `ManifestFetcher` — fetch + validate the manifest only (no extension code runs).
 *   2. `Activator` — import the entry module and run `activate()`.
 */
type ManifestResult = { manifest: ExtensionManifest } | ExtensionLoadError;
...
type Activator = (
  ...
  makeApi: ApiFactory,
  cacheBust?: string,
) => Promise<LoadedExtension | ExtensionLoadError>;

/**
 * When several sources provide the same extension id, only one may be active; the
 * higher-priority one (lower number) wins and the rest are shadowed. Local dev
 * extensions beat a remotely-added URL, which beats the bundled built-in — so a
 ...
 */
const KIND_PRIORITY: Record<ExtensionSourceKind, number> = { local: 0, url: 1, builtin: 2 };

/**
 * Source paths the backend reserves for its own dynamic extension mounts
 * (`/extensions/local/…`, `/extensions/from-workspace/…`). A built-in must never live
 ...
 */
const RESERVED_BUILTIN_PATHS: ReadonlySet<string> = new Set(["/extensions/local", "/extensions/from-workspace"]);
 ...
 * remote URL must not stall built-in/local extensions forever, so it loses the
 * race after this timeout and the others resolve without it.
 */
const MANIFEST_FETCH_TIMEOUT_MS = 5000;

/**
 * A built-in extension shipped in the app bundle. `path` is the app-origin-relative
 * source it loads from (served from `public/extensions/<id>/`). `disabledByDefault`
 * ships the extension off until the user opts in via the settings toggle — use it
 ...
 * across launches (see `extensionEnabledSourcesAtom`).
 */
type BuiltinSource = { path: string; disabledByDefault?: boolean };

/**
 * Built-in extension sources, loaded on boot (when the extensions flag is
 * on) regardless of the user's saved list, and not removable from the settings
 * UI. They serve from `public/extensions/<id>/`.
 */
const BUILTIN_SOURCES: ReadonlyArray<BuiltinSource> = [
  { path: "/extensions/sculpty", disabledByDefault: true },
  { path: "/extensions/pomodoro", disabledByDefault: true },
  { path: "/extensions/buster", disabledByDefault: true },
  { path: "/extensions/linear-issue" },
];

/**
 * Path marker identifying an agent-served *dev* extension mount. The backend serves
 * an extension pushed live from a workspace under `/extensions/local/dev/<ws>/<id>/`;
 * a regular installed local extension lives at the top-level `/extensions/local/<id>/`.
 * Used to render the "dev" badge and to set a snapshot's `origin` to `dev`.
 */
export const DEV_EXTENSION_PATH_MARKER = "/extensions/local/dev/";

/** localStorage namespace prefix for an extension's persisted settings (see `useExtensionSetting`). */
const EXTENSION_CONFIG_KEY_PREFIX = "sculptor-extension:";

/** An extension the backend discovered in the Sculptor extensions directory (the data folder's `extensions/`). */
export type LocalExtensionRef = { id: string; manifestUrl: string };

/**
 * Ask the backend which extensions live in the Sculptor extensions directory. Read-only, so
 ...
 */
const discoverLocalExtensionsFromBackend = async (): Promise<ReadonlyArray<LocalExtensionRef>> => {
  const { data } = await getLocalExtensions({ meta: { skipWsAck: true } });
  return data ?? [];
};
...
export const validateManifest = (manifest: ExtensionManifest): Error | null => {
  ...
  if (major !== HOST_SDK_VERSION) {
    return new Error(`Extension requires SDK major ${major}, host provides ${HOST_SDK_VERSION}`);
  }
  return null;
};
...
/**
 * A human-readable reason a `load`/`reload` did not end with the extension loaded,
 * or `undefined` when every affected extension is `loaded`. Used to set the command
 * result's `ok`, so the CLI reports failure (and exits non-zero) for an extension
 * that uploaded and dispatched fine but then errored at validate/import/activate.
 */
const describeLoadFailure = (extensions: ReadonlyArray<ExtensionSnapshot>): string | undefined => {
  if (extensions.length === 0) return "load produced no extension state";
  const failed = extensions.find((extension) => extension.status !== "loaded");
  if (failed === undefined) return undefined;
  if (failed.status === "error") {
    const where = failed.errorPhase ? ` (${failed.errorPhase})` : "";
    return `extension "${failed.extensionId}" failed to load${where}: ${failed.errorMessage ?? "unknown error"}`;
  }

  if (failed.status === "shadowed") {
    return `extension "${failed.extensionId}" did not load: its id is already provided by ${failed.activeSource ?? "another source"}`;
  }
  return `extension "${failed.extensionId}" did not load (status: ${failed.status})`;
};
 ...
 * source (`/extensions/foo/manifest.json`) resolves against the app origin; an
 * absolute cross-origin source (`http://127.0.0.1:8765/foo/manifest.json`)
 * resolves `entry` against that origin, so an extension served from a local dev
...
/** Phase 1: fetch + validate the manifest (no extension code runs). */
...
const activateFromNetwork: Activator = async (manifestUrl, manifest, makeApi, cacheBust) => {
  ...
  try {
    mod = (await import(/* @vite-ignore */ importUrl)) as ExtensionModule;
  } catch (e) {
  ...
  if (typeof mod.default !== "function") {
    return {
      manifest,
      phase: "activate",
      error: new Error("Extension entry has no default-exported activate() function"),
    };
  }
...
/**
 * Owns the runtime extension lifecycle: bootstrapping built-in plus persisted
 * user sources, and add/remove/reload of individual sources.
 *
 * One instance exists per page load (`extensionManager` below). Extensions are
...
export class ExtensionManager {
  ...
  private readonly discoverLocal: () => Promise<ReadonlyArray<LocalExtensionRef>>;
  ...
  /** Normalized source strings discovered in the Sculptor extensions directory this session. */
  private readonly localSources = new Set<string>();
  private readonly disposersBySource = new Map<string, Array<() => void>>();
  /** Extension id each source has claimed (set when claimed, before activation). Inverse of activeByExtensionId. */
  private readonly extensionIdBySource = new Map<string, string>();
  /** Which source currently owns each extension id — at most one; the active (or activating) source. */
  private readonly activeByExtensionId = new Map<string, string>();
  ...
  constructor(options?: {
    ...
    this.discoverLocal = options?.discoverLocal ?? discoverLocalExtensionsFromBackend;
      ...
      (options?.builtinSources ?? BUILTIN_SOURCES)
        ...
        .filter(([path]) => {
          if (RESERVED_BUILTIN_PATHS.has(path)) {
            console.error(`[extensions] ignoring built-in at reserved path "${path}"`);
            return false;
          }
          return true;
        }),
  ...
  private kindOf(source: string): ExtensionSourceKind {
  ...
  /**
   * The absolute manifest URL to fetch/import for a source. Local extensions are
  ...
  /** Record `source` as the active owner of `extensionId` (claimed before activation). */
  private claim(extensionId: string, source: string): void {
    this.activeByExtensionId.set(extensionId, source);
    this.extensionIdBySource.set(source, extensionId);
  }

  /** Drop `source`'s claim on whatever extension id it owns, if any. */
  private release(source: string): void {
    const extensionId = this.extensionIdBySource.get(source);
    if (extensionId === undefined) return;
    this.extensionIdBySource.delete(source);
    if (this.activeByExtensionId.get(extensionId) === source) {
      this.activeByExtensionId.delete(extensionId);
    }
  }

  /**
   * Loads built-in, persisted-user, and discovered-local sources, resolving any
   * that provide the same extension id down to a single active one. Runs once per
  ...
  private async bootstrapAsync(store: JotaiStore): Promise<void> {
    ...
    const candidates: Array<{ source: string; kind: ExtensionSourceKind }> = [];
    const addCandidate = (source: string, kind: ExtensionSourceKind): void => {
    ...
    for (const s of store.get(extensionSourcesAtom)) addCandidate(normalizeSource(s), "url");
    ...
    const toFetch: Array<{ source: string; kind: ExtensionSourceKind }> = [];
    ...
    // Phase 2: drop fetch failures, group the rest by extension id, resolve each.
    const byId = new Map<string, Array<{ source: string; kind: ExtensionSourceKind; manifest: ExtensionManifest }>>();
  ...
  /**
   * Discover local extension sources (the Sculptor extensions directory) and register them so `kindOf` knows
   * they're local. Returns [] (logged) on discovery failure so the rest of
   * bootstrap proceeds. Each is stored as the RELATIVE extension directory (e.g.
   * `/extensions/local/foo`) — a port-stable identity; it's resolved against the
   * backend origin at load time (see `manifestUrlForLoad`).
   */
  private async discoverLocalCandidates(): Promise<Array<{ source: string; kind: ExtensionSourceKind }>> {
    let refs: ReadonlyArray<LocalExtensionRef>;
    try {
      refs = await this.discoverLocal();
    } catch (e) {
      console.error("[extensions] failed to discover local extensions", e);
      return [];
    }
    const out: Array<{ source: string; kind: ExtensionSourceKind }> = [];
    for (const ref of refs) {
      // Strip the manifest filename so the source is the extension directory:
      ...
      if (!source) {
        console.error(`[extensions] skipping local extension "${ref.id}" — empty manifest URL "${ref.manifestUrl}"`);
        continue;
      }
  ...
  /**
   * Re-scan the Sculptor extensions directory on demand (the settings Refresh button) and
   * reconcile against what's currently tracked, so an extension dropped in (or a
   ...
   *   so that choice is visible and re-applied if the extension returns.
  ...
  async refreshLocalSources(store: JotaiStore): Promise<void> {
    ...
    try {
      ...
      console.error("[extensions] refresh: failed to discover local extensions", e);
      return;
    }
    ...
    // interactive path — if another source already holds the extension id it
    ...
    for (const source of discovered) {
      this.localSources.add(source);
      const status = store.get(extensionSourceStatesAtom)[source]?.status;
    ...
    for (const source of [...this.localSources]) {
      ...
      const status = store.get(extensionSourceStatesAtom)[source]?.status;
      if (status === "loaded") continue; // leave a still-loaded extension running
  ...
  private hasPersistedChoice(store: JotaiStore, source: string): boolean {
    return (
      store.get(extensionDisabledSourcesAtom).includes(source) ||
      store.get(extensionEnabledSourcesAtom).includes(source)
    );
  }
  ...
  private async resolveGroup(
    store: JotaiStore,
    group: Array<{ source: string; kind: ExtensionSourceKind; manifest: ExtensionManifest }>,
  ): Promise<void> {
    ...
    // stability — e.g. two local extensions with the same id resolve to the one the
   ...
   * else loads.
   */
  private isDisabled(store: JotaiStore, source: string, kind: ExtensionSourceKind = this.kindOf(source)): boolean {
    if (store.get(extensionDisabledSourcesAtom).includes(source)) return true;
    if (store.get(extensionEnabledSourcesAtom).includes(source)) return false;
   ...
   * choice survives a reload — this is the opt-out for built-in extensions and the
  ...
  async setSourceEnabled(store: JotaiStore, source: string, enabled: boolean): Promise<void> {
    ...
    const status = store.get(extensionSourceStatesAtom)[normalized]?.status;
    ...
    const disabled = store.get(extensionDisabledSourcesAtom).filter((s) => s !== normalized);
    const enabledOverrides = store.get(extensionEnabledSourcesAtom).filter((s) => s !== normalized);

    if (enabled) {
      if (status === "loaded") return; // already active
      store.set(extensionDisabledSourcesAtom, disabled);
      store.set(extensionEnabledSourcesAtom, [...enabledOverrides, normalized]);
      // (Re)attempt the load. If another source still holds this extension id, the
      ...
      store.set(extensionDisabledSourcesAtom, [...disabled, normalized]);
      store.set(extensionEnabledSourcesAtom, enabledOverrides);
      this.unloadSource(normalized); // frees its extension id so a shadowed sibling can be enabled
  ...
  async addSource(store: JotaiStore, rawSource: string): Promise<void> {
    // Normalize (trim + drop trailing slashes) so `/extensions/foo` and
    // `/extensions/foo/` can't be stored as distinct sources or slip past the
    // duplicate/builtin guard below.
    const source = normalizeSource(rawSource);
    if (!source) return;
    const existing = store.get(extensionSourcesAtom);
    ...
    store.set(extensionSourcesAtom, [...existing, source]);
  ...
  removeSource(store: JotaiStore, source: string): void {
    store.set(
      extensionSourcesAtom,
      store.get(extensionSourcesAtom).filter((s) => s !== source),
    );
    ...
    store.set(
      extensionDisabledSourcesAtom,
      store.get(extensionDisabledSourcesAtom).filter((s) => s !== source),
    );
    store.set(
      extensionEnabledSourcesAtom,
      store.get(extensionEnabledSourcesAtom).filter((s) => s !== source),
    );
   ...
   * A caller may pass an explicit `cacheBust` token (e.g. forwarded from an
   * extension command); absent one, a timestamp forces a fresh fetch.
   */
   ...
   * extension id. If another source already holds that id, settle into a "shadowed"
  ...
  private async loadSource(
    ...
    if ("phase" in result) {
      console.error(`Extension manifest load failed (${result.phase}) for "${source}"`, result.error);
      this.setSourceState(store, source, {
        ...
        extensionId: result.phase === "manifest" ? undefined : result.manifest.id,
      });
      return;
    }

    const manifest = result.manifest;
    const owner = this.activeByExtensionId.get(manifest.id);
  ...
  /**
   * Phase 2 for a source whose extension id is already claimed: import + activate,
   * honoring the sequence token so a load superseded mid-flight rolls back and
   * commits nothing. Returns whether the extension ended up loaded.
   */
  private async activateClaimed(
    ...
    let outcome: LoadedExtension | ExtensionLoadError;
    try {
    ...
    if ("phase" in outcome) {
      console.error(`Extension load failed (${outcome.phase}) for "${source}"`, outcome.error);
      // An extension can register panels/settings/overlays and *then* throw (or
    ...
    // The extension id was already recorded by `claim`; activation just confirms it.
  ...
  private unloadSource(source: string): void {
    ...
    this.release(source); // free its extension id so a shadowed sibling can claim it
  ...
  private runDisposers(source: string, disposers: ReadonlyArray<() => void>): void {
    for (const dispose of disposers) {
      try {
        ...
        console.error(`Extension disposer threw for "${source}"`, e);
      }
    }
  }

  /** Builds the per-extension `api` handed to `activate()`, backed by the given store. */
  private makeApi(store: JotaiStore, manifest: ExtensionManifest, loadDisposers: Array<() => void>): ExtensionHostApi {
    return {
      registerPanel: (panel: ExtensionPanelDefinition): (() => void) => {
        // Wrap the extension's component in the error boundary plus context
        // providers exposing the extension id (for settings hooks) and the
        // current workspace id (read fresh per render from the route params).
        const ExtensionComponent = panel.component;
        const Wrapped = (): ReactElement | null => {
          const { workspaceID } = useWorkspacePageParams();
          if (!workspaceID) return null;
          return (
            // Attribute crashes to the extension, not the individual panel — one
            // extension can contribute several panels, and the error UI/logs should
            // name the extension that owns them.
            <ExtensionErrorBoundary extensionId={manifest.id} extensionName={manifest.name}>
              <ExtensionContext.Provider value={{ extensionId: manifest.id }}>
                <WorkspaceExtensionContext.Provider value={{ workspaceId: workspaceID }}>
                  <ExtensionComponent />
                </WorkspaceExtensionContext.Provider>
              </ExtensionContext.Provider>
            </ExtensionErrorBoundary>
          );
        };
        Wrapped.displayName = `ExtensionPanel(${panel.id})`;
        const wrappedPanel: ExtensionPanelDefinition = { ...panel, component: Wrapped, extensionId: manifest.id };
        ...
        store.set(extensionPanelsAtom, (prev) => [...prev.filter((p) => p.id !== panel.id), wrappedPanel]);
        const undo = (): void => {
          store.set(extensionPanelsAtom, (prev) => prev.filter((p) => p !== wrappedPanel));
        };
        loadDisposers.push(undo);
        return undo;
      },
      registerSettings: (component: ComponentType): (() => void) => {
        store.set(extensionSettingsComponentsAtom, (prev) => ({ ...prev, [manifest.id]: component }));
        const undo = (): void => {
          store.set(extensionSettingsComponentsAtom, (prev) => {
      ...
      registerOverlay: (overlay: OverlayDefinition): (() => void) => {
        // Like a panel, wrap in the error boundary and ExtensionContext, but with
        // no WorkspaceExtensionContext: an overlay is app-global, so it reads the
        // current workspace through the SDK hooks instead of a fixed context.
        const ExtensionComponent = overlay.component;
        // Attribute crashes to the owning extension (manifest), matching the panel
        ...
        Wrapped.displayName = `ExtensionOverlay(${overlay.id})`;
        const entry = { id: overlay.id, component: Wrapped, extensionId: manifest.id };

        // Replace-by-id; undo by instance (see the panel undo above).
        store.set(extensionOverlaysAtom, (prev) => [...prev.filter((o) => o.id !== overlay.id), entry]);
        const undo = (): void => {
          store.set(extensionOverlaysAtom, (prev) => prev.filter((o) => o !== entry));
        };
        loadDisposers.push(undo);
        return undo;
      },
      registerWorkspaceWidget: (widget: WorkspaceWidgetDefinition): (() => void) => {
        ...
        // ExtensionContext, and a WorkspaceExtensionContext whose id is read fresh per
        ...
        const ExtensionComponent = widget.component;
        const Wrapped = (): ReactElement | null => {
          const { workspaceID } = useWorkspacePageParams();
          if (!workspaceID) return null;
          return (
        ...
        Wrapped.displayName = `ExtensionWorkspaceWidget(${widget.id})`;
        ...
        // Replace-by-id; undo by instance (see the panel undo above).
        store.set(extensionWorkspaceWidgetsAtom, (prev) => [...prev.filter((w) => w.id !== widget.id), entry]);
        const undo = (): void => {
          store.set(extensionWorkspaceWidgetsAtom, (prev) => prev.filter((w) => w !== entry));
        };
        loadDisposers.push(undo);
        return undo;
      },
      registerHomeView: (view: HomeViewDefinition): (() => void) => {
        // The recent-workspaces view is the built-in the host always offers; an
        // extension can't claim its id, or the switcher would have two options with
        // the same value. Reject up front (rather than silently dropping it in
        // the options atom) so the extension author gets a reason it didn't appear.
        if (view.id === BUILTIN_HOME_VIEW_ID) {
          console.warn(
            `Extension "${manifest.id}" tried to register a home view with the reserved id "${BUILTIN_HOME_VIEW_ID}"; ignoring it.`,
          );
          return () => {};
        }
        // App-global like an overlay: wrap in the error boundary and
        // ExtensionContext, but no WorkspaceExtensionContext — the homepage is not
        // scoped to a workspace, so the view reads app state through SDK hooks.
        const ExtensionComponent = view.component;
        // Attribute crashes to the owning extension (manifest), matching the other
        ...
        Wrapped.displayName = `ExtensionHomeView(${view.id})`;
        const entry = { id: view.id, title: view.title, icon: view.icon, component: Wrapped };

        // Replace-by-id; undo by instance (see the panel undo above).
        store.set(extensionHomeViewsAtom, (prev) => [...prev.filter((v) => v.id !== view.id), entry]);
        const undo = (): void => {
          store.set(extensionHomeViewsAtom, (prev) => prev.filter((v) => v !== entry));
        };
        loadDisposers.push(undo);
        return undo;
      },
  ...
  private setSourceState(store: JotaiStore, source: string, state: ExtensionSourceState | undefined): void {
    store.set(extensionSourceStatesAtom, (prev) => {
      const next = { ...prev };
      if (state === undefined) {
        delete next[source];
      } else {
  ...
  /**
   * Run an agent-issued `sculpt extension <op>` against this renderer's extension
   ...
   */
  async handleExtensionCommand(store: JotaiStore, action: ExtensionCommandUiAction): Promise<ExtensionCommandResult> {
    const base = {
    ...
    try {
      const extensions = await this.runExtensionCommand(store, action);
      // A load/reload whose extension fails at validate/import/activate settles into
      // an error snapshot rather than throwing, so `ok` must reflect the extension's
      // final status — otherwise the CLI reports success for an extension that never
      // loaded (and exits 0).
      const loadFailure = action.op === "load" || action.op === "reload" ? describeLoadFailure(extensions) : undefined;
      if (loadFailure !== undefined) return { ...base, ok: false, error: loadFailure, extensions };
      return { ...base, ok: true, extensions };
    } catch (e) {
      return { ...base, ok: false, error: e instanceof Error ? e.message : String(e), extensions: [] };
    }
  }

  /** Dispatch a single extension command op; throws on failure (wrapped by `handleExtensionCommand`). */
  private async runExtensionCommand(
    store: JotaiStore,
    action: ExtensionCommandUiAction,
  ): Promise<Array<ExtensionSnapshot>> {
    switch (action.op) {
      case "load": {
        ...
        // normal load path so the extension registers, activates, and shows up live
        // in `extensionSourceStatesAtom`.
        const status = store.get(extensionSourceStatesAtom)[normalized]?.status;
        if (status === "loaded") {
          ...
          // Persisted across restart via `extensionSourcesAtom`. `addSource` no-ops
          // if the URL is already tracked, so fall back to an explicit load then.
          await this.addSource(store, normalized);
          if (store.get(extensionSourceStatesAtom)[normalized]?.status === undefined) {
      ...
      case "reload": {
        if (!action.extensionId) throw new Error("reload requires an extensionId");
        // Resolve by id or source so a failed load can be retried by id after a
        // fix, not just an already-active extension.
        const [source] = this.resolveSources(store, action.extensionId);
        if (source === undefined) throw new Error(`extension "${action.extensionId}" is not loaded in this renderer`);
        await this.reloadSource(store, source, action.cacheBust ?? undefined);
        return this.snapshotForSource(store, source);
      }

      case "unload": {
        if (!action.extensionId) throw new Error("unload requires an extensionId");
        ...
        const sources = this.resolveSources(store, action.extensionId);
        if (sources.length === 0) throw new Error(`extension "${action.extensionId}" is not loaded in this renderer`);
        for (const source of sources) {
          this.unloadSource(source);
          this.setSourceState(store, source, undefined);
        }
        // The extension is gone; report the now-removed id with a "missing" status
        // so the CLI can confirm which extension was unloaded.
        return [
          {
            ...
            configKeys: this.configKeysForExtensionId(action.extensionId),
          },
        ];
      }

      case "inspect": {
        if (!action.extensionId) throw new Error("inspect requires an extensionId");
        // Match by id or by source path, so a failed load (reported under its
        // path when no id is known) is still inspectable.
        return this.snapshotAll(store).filter(
          (p) => p.extensionId === action.extensionId || p.source === action.extensionId,
        );
      }
      case "list":
        return this.snapshotAll(store);
      default:
        throw new Error(`unknown extension op "${String(action.op)}"`);
    }
  }

  /**
   * The extension id a source is known by: the manifest id when loaded/shadowed,
   * the retained id on a failed load, else the manager's claim map, else the
   * source string itself.
   */
  private extensionIdForState(source: string, state: ExtensionSourceState): string {
    if (state.status === "loaded" || state.status === "shadowed") return state.manifest.id;
    if (state.status === "error" && state.extensionId !== undefined) return state.extensionId;
    return this.extensionIdBySource.get(source) ?? source;
  }

  /**
   * Every source addressable by `target` — matched on its extension id (active,
  ...
  private resolveSources(store: JotaiStore, target: string): Array<string> {
    ...
    return Object.keys(states).filter(
      (source) => source === target || this.extensionIdForState(source, states[source]) === target,
    );
  }
   ...
   */
  private originOf(source: string): ExtensionSnapshot["origin"] {
    if (this.builtinByPath.has(source)) return "builtin";
    if (source.includes(DEV_EXTENSION_PATH_MARKER)) return "dev";
    if (/^https?:\/\//i.test(source)) return "url";
    return "installed";
  }

  /**
   * Persisted setting key NAMES (never values — they may be credentials) for an
   * extension, read from the `sculptor-extension:<extensionId>:<key>` localStorage
   * namespace. Returns just the `<key>` suffixes.
   */
  private configKeysForExtensionId(extensionId: string): Array<string> {
    if (!extensionId) return [];
    const prefix = `${EXTENSION_CONFIG_KEY_PREFIX}${extensionId}:`;
  ...
  /**
   * Names-only registration summary for an extension: the panels and overlays it
   * registered (each entry carries its owning `extensionId`) and whether it
   * contributed a settings component.
   */
  private registrationsForExtensionId(store: JotaiStore, extensionId: string): ExtensionRegistrations {
    const panels = store
      .get(extensionPanelsAtom)
      .filter((p) => p.extensionId === extensionId)
      .map((p) => p.displayName || p.id);
    const hasSettings = extensionId in store.get(extensionSettingsComponentsAtom);
    const overlays = store
      .get(extensionOverlaysAtom)
      .filter((o) => o.extensionId === extensionId)
      .map((o) => o.id);
    return { panels, hasSettings, overlays };
  }

  /** Build a snapshot for a single source (used by load/reload). */
  private snapshotForSource(store: JotaiStore, source: string): Array<ExtensionSnapshot> {
    const state = store.get(extensionSourceStatesAtom)[source];
  ...
  private snapshotAll(store: JotaiStore): Array<ExtensionSnapshot> {
    const states = store.get(extensionSourceStatesAtom);
    return Object.entries(states).map(([source, state]) => this.snapshotFrom(store, source, state));
  }

  /** Assemble one redacted `ExtensionSnapshot` from a source's live state. */
  private snapshotFrom(store: JotaiStore, source: string, state: ExtensionSourceState): ExtensionSnapshot {
    const extensionId = this.extensionIdForState(source, state);
    const snapshot: ExtensionSnapshot = {
      extensionId,
      source,
      status: state.status,
      origin: this.originOf(source),
      configKeys: this.configKeysForExtensionId(extensionId),
    };
    ...
    // Registrations are only meaningful once an extension is live in this renderer.
    if (state.status === "loaded") {
      snapshot.registrations = this.registrationsForExtensionId(store, extensionId);
    }
    return snapshot;
  }

  /** Membership test over the live extension ids without allocating — the guard below runs per dev cache event. */
  private isRegisteredExtensionId(candidate: string): boolean {
    for (const extensionId of this.extensionIdBySource.values()) {
      if (extensionId === candidate) return true;
    }
    return false;
  }
   ...
   * host keys start with the reserved "sculptor" prefix, extension keys with the
   * extension id. Anything else is a query that invalidation can't reach by
   * namespace — warn so the author (host or extension) fixes the key shape.
   */
  private installQueryKeyNamespaceGuard(): void {
    ...
    queryClient.getQueryCache().subscribe((event) => {
      ...
      if (typeof first === "string" && this.isRegisteredExtensionId(first)) return;
      console.warn(
        `[extensions] query key outside any namespace: ${JSON.stringify(event.query.queryKey)} — ` +
          `host keys must start with "${SCULPTOR_QUERY_KEY_PREFIX}", extension keys with the extension id.`,
      );
    });
  }
}
