...
import { useExtensionContext } from "../ExtensionContext.tsx";
import { useWorkspaceExtensionContext, WorkspaceExtensionContext } from "../WorkspaceContext.tsx";
...
export function useCurrentWorkspace<T = WorkspaceView | null>(
  ...
  const panelContext = useContext(WorkspaceExtensionContext);
...
// One persisted atom per (extension, key). atomFamily caches by the full storage
// key; getOnInit reads localStorage synchronously so the value is present on
// first render instead of flashing the default.
const extensionSettingAtomFamily = atomFamily((storageKey: string) =>
  atomWithStorage<string>(storageKey, "", undefined, { getOnInit: true }),
);

// The localStorage namespace for one of an extension's settings. Shared by the
// single- and multi-key hooks so the key shape can't drift between them.
const settingStorageKey = (extensionId: string, key: string): string => `sculptor-extension:${extensionId}:${key}`;
 ...
 */
export const useExtensionSetting = (key: string): [string, (value: string) => void] => {
  const { extensionId } = useExtensionContext();
  return useAtom(extensionSettingAtomFamily(settingStorageKey(extensionId, key)));
};
 ...
 * atoms.
 */
export const useExtensionSettings = (keys: ReadonlyArray<string>): ReadonlyMap<string, string> => {
  const { extensionId } = useExtensionContext();
  ...
  // time. Extension keys are `<name>:<id>`-style and never contain a newline, so it
  ...
  const mapAtom = useMemo(
    () =>
      atom((get): ReadonlyMap<string, string> => {
        ...
        return new Map(
          requestedKeys.map((key) => [key, get(extensionSettingAtomFamily(settingStorageKey(extensionId, key)))]),
        );
      }),
    [extensionId, encodedKeys],
  );
 ...
 */
export const useSetExtensionSetting = (): ((key: string, value: string) => void) => {
  const { extensionId } = useExtensionContext();
  const store = useStore();
  return useCallback(
    (key: string, value: string): void => {
      store.set(extensionSettingAtomFamily(settingStorageKey(extensionId, key)), value);
    },
    [extensionId, store],
  );
};

// The settings hooks under their pre-rename SDK names. Extensions compiled
// against the older SDK import these from the runtime stub at load time, so the
// aliases must stay exported (and re-exported from the SDK barrel) for those
// bundles to keep loading.
/** @deprecated Use {@link useExtensionSetting}. Kept for extensions compiled against the pre-rename SDK. */
export const usePluginSetting = useExtensionSetting;
/** @deprecated Use {@link useExtensionSettings}. Kept for extensions compiled against the pre-rename SDK. */
export const usePluginSettings = useExtensionSettings;
/** @deprecated Use {@link useSetExtensionSetting}. Kept for extensions compiled against the pre-rename SDK. */
export const useSetPluginSetting = useSetExtensionSetting;
...
export const useWorkspaceTasks = (): ReadonlyArray<CodingAgentTaskView> | undefined => {
  const { workspaceId } = useWorkspaceExtensionContext();
  const tasks = useAtomValue(tasksArrayAtom);
  // Memoize so extension authors get a stable array identity across renders (safe
