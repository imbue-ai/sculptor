...
} from "../../vite-plugins/extension-runtime-stubs.ts";
...
describe("extensionRuntimeStubs (host-versions virtual module)", () => {
  it("resolves and loads the virtual module with the real installed versions", async () => {
    const vitePlugin: Plugin = extensionRuntimeStubs();
    // Point package resolution at this frontend project.
    callHook(vitePlugin.configResolved, { root: process.cwd() });

    const resolved = callHook(vitePlugin.resolveId, HOST_VERSIONS_MODULE_ID) as string;
    expect(resolved).toBe("\0" + HOST_VERSIONS_MODULE_ID);

    const code = (await callHook(vitePlugin.load, resolved)) as string;
  ...
  it("ignores unrelated module ids", () => {
    const vitePlugin: Plugin = extensionRuntimeStubs();
    expect(callHook(vitePlugin.resolveId, "some-other-module")).toBeUndefined();
  });
});
...
describe("collectStubs (real installed namespaces)", () => {
  ...
  it("generates a stub per module with names from the real namespaces", async () => {
    ...
    expect(
      ...
      "Generated stub set changed. If you added or removed a shared package, update " +
        "RUNTIME_MODULES in vite-plugins/extension-runtime-stubs.ts, this expected list, " +
    ...
    const sdk = stubs.get("sculptor-extension-sdk.js") as string;
    expect(sdk).toContain("export const useExtensionSetting = host_sdk.useExtensionSetting;");
    expect(sdk).toContain("export const PanelHeader = host_sdk.PanelHeader;");

    // The pre-rename hook names stay in the stub: extensions compiled against
    // the older SDK import them at runtime, so dropping them would break every
    // such bundle at load.
    expect(sdk).toContain("export const usePluginSetting = host_sdk.usePluginSetting;");
    expect(sdk).toContain("export const usePluginSettings = host_sdk.usePluginSettings;");
    expect(sdk).toContain("export const useSetPluginSetting = host_sdk.useSetPluginSetting;");
  });

  it("keeps both SDK specifiers external for extension builds", () => {
    // The import map resolves the current and the pre-rename SDK specifier to
    // the same stub, so both must stay external — a bundle that inlined either
    // would stop sharing the host's singletons.
    expect(RUNTIME_MODULE_SPECIFIERS).toContain("@sculptor/extension-sdk");
    expect(RUNTIME_MODULE_SPECIFIERS).toContain("@sculptor/plugin-sdk");
  });
  ...
  // below). Not exhaustive — full SDK surface coverage belongs to extension SDK
