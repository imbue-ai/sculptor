#!/usr/bin/env bash
# Roll up the plugin SDK's public declarations into a single .d.ts shipped
# inside the build-sculptor-plugin skill. The skill travels with the app to
...
#
# Usage: generate-plugin-sdk-dts.sh [out-file]
...
out_file="${1:-../sculptor-plugin/skills/build-sculptor-plugin/sdk.d.ts}"
...
pnpm exec dts-bundle-generator \
  ...
  --out-file "$tmp_file" \
  src/plugins/sdk/index.ts >/dev/null

{
  echo "// GENERATED FILE - do not edit. Regenerate with: just generate-plugin-sdk-dts"
  echo "// The public contract of \"@sculptor/plugin-sdk\" (host SDK major 1), rolled up"
  echo "// from sculptor/frontend/src/plugins/sdk/ in the Sculptor repo."
  cat "$tmp_file"
} > "$out_file"
