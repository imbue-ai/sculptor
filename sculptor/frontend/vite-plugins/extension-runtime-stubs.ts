...
/**
 * Generates the extension system's `/extension-runtime/*.js` stubs at build
 * time instead of hand-maintaining them (SCU-1488).
 *
 * Each stub resolves a bare-specifier import in an extension bundle (via the import
 * map in `index.html`) to the host's singleton instance of a shared package,
 * read from `window.__SCULPTOR_HOST__` (populated by `src/extensions/hostRuntime.ts`).
 * Hand-written stubs enumerated a curated subset of each package's exports, so an
 * extension importing an un-listed name silently got `undefined` at runtime. Here we
 * derive the export list from the *actual* installed module namespace, so the
 * stub mirrors exactly what the host provides.
 *
 * The extension serves the stubs from `/extension-runtime/<file>` in dev and emits them
 ...
 * (`virtual:sculptor/extension-host-versions`) so the loader can later validate an
 * extension manifest's declared peer ranges against the real versions.
 */
...
export type RuntimeModuleConfig = {
  /** Output filename under `/extension-runtime/`. */
  file: string;
  /**
  ...
  /**
   * Names deliberately withheld from extensions even though the namespace has them
...
const SDK_SPECIFIER = "@sculptor/extension-sdk";
/**
 * Extensions compiled against the pre-rename SDK import this specifier. The
 * import map keeps an entry for it (pointing at the same stub), so it must stay
 * external in extension builds too — otherwise a bundle built from older
 * sources would inline its own SDK copy instead of sharing the host's.
 */
const LEGACY_SDK_SPECIFIER = "@sculptor/plugin-sdk";
const SDK_SOURCE_PATH = "src/extensions/sdk/index.ts";
...
const RUNTIME_MODULES: ReadonlyArray<RuntimeModuleConfig> = [
  ...
  { file: "jotai.js", sources: [{ specifier: "jotai", hostKey: "jotai" }] },
  {
    ...
    // Extensions share the host's QueryClient (resolved via context — extension panels
    // render under the host's QueryClientProvider). Withhold the means to build a
    // second one: a nested client would cut host components rendered inside an
    // extension subtree off from the shared cache.
  ...
  { file: "sculptor-extension-sdk.js", sources: [{ specifier: SDK_SPECIFIER, hostKey: "sdk" }] },
];

/**
 * The bare specifiers the import map resolves to host singletons — i.e. exactly
 * what a compiled extension bundle must mark external so it shares the host's
 * instances instead of bundling its own copy. Derived from `RUNTIME_MODULES`
 * so an extension build's externals can't drift from what the stubs provide.
 * The legacy SDK specifier rides along because the import map still resolves it.
 */
export const RUNTIME_MODULE_SPECIFIERS: ReadonlyArray<string> = [
  ...new Set([
    ...RUNTIME_MODULES.flatMap((module) => module.sources.map((source) => source.specifier)),
    LEGACY_SDK_SPECIFIER,
  ]),
];
...
export const HOST_VERSIONS_MODULE_ID = "virtual:sculptor/extension-host-versions";
const RESOLVED_HOST_VERSIONS_MODULE_ID = "\0" + HOST_VERSIONS_MODULE_ID;

const GENERATED_HEADER =
  "// @generated by vite-plugins/extension-runtime-stubs.ts (SCU-1488). DO NOT EDIT.\n" +
...
export const renderStub = (
  ...
  lines.push(
    '  throw new Error("Sculptor extension runtime: window.__SCULPTOR_HOST__ missing " + __missing.join(", ") + ".");',
  );
  lines.push("}");
...
const readInstalledVersion = (root: string, pkg: string): string => {
  ...
  if (typeof manifest.version !== "string") {
    throw new Error(`extension-runtime-stubs: no version for "${pkg}" at ${manifestPath}`);
  }
...
export const extensionRuntimeStubs = (): Plugin => {
  ...
  return {
    name: "sculptor:extension-runtime-stubs",
    ...
    configureServer(server): void {
      server.middlewares.use((req, res, next) => {
        ...
        const match = url.match(/^\/extension-runtime\/([\w.-]+\.js)$/);
        if (!match) {
          next();
          return;
        }
    ...
    async generateBundle(): Promise<void> {
      ...
      for (const [file, source] of stubs) {
        ...
        // references — /extension-runtime/<file> — stable and unhashed.
        this.emitFile({ type: "asset", fileName: `extension-runtime/${file}`, source });
      }
    },
  };
};
