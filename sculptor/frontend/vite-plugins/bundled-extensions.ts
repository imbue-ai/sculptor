...
import { RUNTIME_MODULE_SPECIFIERS } from "./extension-runtime-stubs.ts";

/**
 * Builds the bundled extensions whose source is TypeScript/TSX (and so needs
 * compiling) as part of the host build, emitting each bundle into
 * `public/extensions/<id>/` — the same tree the no-build, pure-JS extensions live in.
 *
 * Pure-JS extensions need no build (their source is committed directly under
 * `public/extensions/<id>/`). Wiring the compile into the host build means
 * `npm run build` and the dev server produce the bundle — there's no separate
 * `cd extensions/<id> && npm run build` step, and no second toolchain: the extension
 ...
 * only the extension's own code and shares the host's React, Radix, TanStack
 * Query, etc. at runtime.
 */

/** Extensions built from TS/TSX source at `extensions/<id>/src/index.tsx`. */
const COMPILED_EXTENSION_IDS: ReadonlyArray<string> = ["linear-issue", "openhost-preview-switcher"];

const buildCompiledExtension = async (frontendRoot: string, id: string): Promise<void> => {
  const sourceDir = path.join(frontendRoot, "extensions", id);
  const outDir = path.join(frontendRoot, "public", "extensions", id);
  // A compiled extension is a production artifact: it shares the host's React
  // singleton via the import map, which provides only the prod
  // `react/jsx-runtime` (not `react/jsx-dev-runtime`). It MUST build as
  // production, or the JSX transform emits the dev runtime and extension renders
  ...
  // doesn't provide, so the extension would crash at render with
  ...
  if (bundle.includes("jsxDEV")) {
    throw new Error(
      `Extension "${id}" was built with React's dev JSX transform (found "jsxDEV"). ` +
        `The host provides only the production "react/jsx-runtime", so this would crash at render. ` +
        `The production pin in bundled-extensions.ts is not taking effect — check NODE_ENV handling.`,
    );
  }
...
export const bundledExtensions = (): Plugin => {
  ...
  const buildAll = async (): Promise<void> => {
    for (const id of COMPILED_EXTENSION_IDS) await buildCompiledExtension(frontendRoot, id);
  };
  const buildAllOnce = (): Promise<void> => (startupBuild ??= buildAll());

  return {
    name: "sculptor:bundled-extensions",

    // Build the dev-server bundles HERE, during config resolution — a hook Vite
    // awaits (before it creates the server) — so `public/extensions/<id>/main.js`
    ...
    // HTML as an ES module and the extension's dynamic import fails ("failed:
    ...
    // runs without this extension) can't recurse.
    ...
    async configureServer(server): Promise<void> {
      ...
      const sourceRoot = path.join(frontendRoot, "extensions");
      ...
      server.watcher.on("change", (file: string): void => {
        ...
        void buildAll()
          ...
          .catch((error: unknown) => {
            server.config.logger.error(`bundled-extensions: extension rebuild failed: ${String(error)}`);
          })
          .finally(() => {
            isRebuilding = false;
          });
