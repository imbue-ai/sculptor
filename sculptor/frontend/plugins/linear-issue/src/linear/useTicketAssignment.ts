...
/**
 * The per-workspace plugin-setting key under which a workspace's assigned ticket
 * is stored. Exported so the board can read the same keys the panel writes (via
 * `usePluginSettings`) — the two must agree on the format.
 */
 ...
 * Linear ticket the workspace is for — persisted via the plugin-settings SDK
 ...
 * (via `usePluginSettings`), so a workspace shows under its assigned ticket even
...
export const useTicketAssignment = (workspaceId: string | null): TicketAssignment => {
  const [raw, setRaw] = usePluginSetting(ticketAssignmentKey(workspaceId));
