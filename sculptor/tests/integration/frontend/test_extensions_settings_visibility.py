"""Integration tests for the Extensions settings section and its global toggle.

The Extensions settings section is always present in the sidebar — it hosts the
toggle that globally enables or disables the whole extension system, so it must
stay reachable even when the system is off, to turn it back on. The system is on
by default: the section shows the extension-management UI (the add-source input and
...
def test_extensions_section_present_and_on_by_default(sculptor_instance_: SculptorInstance) -> None:
    """The Extensions section is in the sidebar with the system on by default.
    ...
    expect(settings_page.get_extensions_nav()).to_be_visible()

    extensions = settings_page.click_on_extensions()
    # On by default: the toggle is on and the management UI (add-source input) shows.
    expect(extensions.get_extensions_toggle()).to_have_attribute("data-state", "checked")
    expect(extensions.get_source_input()).to_be_visible()
...
def test_global_toggle_hides_management_ui_live(
    ...
    with sculptor_instance_factory_.spawn_instance() as instance:
        ...
        extensions.set_extensions(enabled=False)
        expect(settings_page.get_extensions_nav()).to_be_visible()
        expect(extensions.get_source_input()).to_have_count(0)

        # Turning it back on reveals the management UI again.
        extensions.set_extensions(enabled=True)
        expect(extensions.get_source_input()).to_be_visible()
