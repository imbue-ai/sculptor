...
def _set_pi_config(factory: SculptorInstanceFactory, pi_value: str, enable_multi_harness: bool = False) -> None:
    ...
    if enable_multi_harness:
        updates["enable_multi_harness"] = True
...
def test_pi_settings_section_visible_with_multi_harness_disabled(
    sculptor_instance_factory_: SculptorInstanceFactory,
) -> None:
    """With enable_multi_harness OFF (the default) the section is still usable —
...
def test_pi_workspace_surfaces_structured_failure_when_pi_unavailable(
    ...
    with sculptor_instance_factory_.spawn_instance() as instance:
        # start_task_and_wait_for_ready enables the multi-harness picker for us.
