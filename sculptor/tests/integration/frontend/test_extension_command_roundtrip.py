"""Integration test for the `sculpt extension` command round-trip against a live renderer.

This is the first e2e coverage of the extension *command bridge*: a POST to the
per-workspace ``extensions/command`` endpoint publishes an ``ExtensionCommandUiAction``
over the per-user stream WebSocket to the connected renderer, which runs the op
through its ``ExtensionManager`` and POSTs a ``ExtensionCommandResult`` back, which the
endpoint returns. The backend endpoints and ``ExtensionManager`` are covered by unit
...
def _agent_loading_populator(folder_path: Path) -> None:
    """Seed the per-test config with agent extension loading enabled.

    ``allow_agent_extension_loading`` is off by default (turning it on lets a
    workspace run frontend code in the UI), and write ops like ``load`` are gated
    on it, so the test must opt in. ``enable_extensions`` is already on by
    ...
    config = load_config(config_path).model_copy(update={"allow_agent_extension_loading": True})
...
def _wait_for_renderer(instance: SculptorInstance, workspace_id: str) -> None:
    ...
    """
    url = f"{instance.backend_api_url.rstrip('/')}/api/v1/workspaces/{workspace_id}/extensions/command"
    ...
    raise AssertionError("renderer stream never connected / responded to an extension command")


@custom_sculptor_folder_populator.with_args(_agent_loading_populator)
def test_extension_command_load_reaches_the_live_renderer(sculptor_instance_factory_: SculptorInstanceFactory) -> None:
    ...
    Settings -> Extensions list shows it.
    """
    manifest = json.loads((_EXAMPLE_EXTENSION_DIR / "manifest.json").read_text())
    ...
    with sculptor_instance_factory_.spawn_instance() as instance:
        ...
        install = _post_json(
            instance,
            f"/api/v1/workspaces/{workspace_id}/extensions/install",
            {
                "extensionId": extension_id,
                # Mirror the CLI's packaging filter: regular files only, no dotfiles.
                "files": [
                    {"path": path.name, "contentBase64": _b64(path.read_text())}
                    for path in sorted(_EXAMPLE_EXTENSION_DIR.iterdir())
                    if path.is_file() and not path.name.startswith(".")
                ],
            },
        )
        manifest_url = install["manifestUrl"]
        assert manifest_url == f"/extensions/local/dev/{workspace_id}/{extension_id}/manifest.json"

        loaded = _post_json(
            instance,
            f"/api/v1/workspaces/{workspace_id}/extensions/command",
        ...
        assert result["extensions"], f"expected the loaded extension in the reply, got {result}"
        extension = result["extensions"][0]
        assert extension["extensionId"] == extension_id, extension
        assert extension["status"] == "loaded", extension
        assert extension["origin"] == "dev", extension

        # The renderer's own Settings -> Extensions list reflects the load reactively
        ...
        extensions.expect_loaded(extension["source"], name=manifest["name"], version=manifest["version"])
