"""Integration test for the bundled Linear example extension (SCU-1552).

Unlike test_extension_loader.py — which exercises the loader against synthetic
fixture-server extensions — this targets the *real* bundled `linear-issue` extension,
the one the host Vite build compiles into `public/extensions/`. It asserts the
built-in source both:

  1. loads — the bundle was fetched, validated, imported, and activated, and
  2. renders its own React — the extension's settings component mounts and shows
     its content.
...
runtime) trips the extension error boundary, so the settings text never appears.

Runs in both browser and electron launch modes, since the extension ships bundled
...
# The built-in source the host registers for the bundled Linear extension.
LINEAR_SOURCE = "/extensions/linear-issue"
# Stable text from the extension's own settings component (LinearSettings). The
# extension error boundary would replace it on a render failure, so asserting it
# confirms the extension's React actually ran.
LINEAR_SETTINGS_TEXT = "Personal API key from Linear"

# Linear's GraphQL endpoint the extension's client posts to (see linear/client.ts).
# Mocked in-page so the widget renders against canned data with no real network.
LINEAR_GRAPHQL_URL = "https://api.linear.app/graphql"
# Test-only testids the extension sets on its contributions.
...
def _mock_linear_graphql(instance: SculptorInstance) -> None:
    """Intercept the extension's Linear GraphQL calls and answer with canned data.
...
def _set_linear_api_key(extensions: PlaywrightExtensionsSettingsElement, key: str) -> None:
    """Enter a Linear API key via the extension's own settings component.

    Going through the real settings input (rather than seeding localStorage)
    drives the SDK ``useExtensionSetting`` write the panel and widget both read, so
    the key is live app-wide by the time the workspace banner mounts.
    """
    extensions.open_source_settings(LINEAR_SOURCE)
    extensions.get_source_row(LINEAR_SOURCE).get_by_placeholder(LINEAR_API_KEY_PLACEHOLDER).fill(key)


def _assert_linear_extension_loads_and_renders(extensions: PlaywrightExtensionsSettingsElement) -> None:
    """The built-in Linear extension loads, then renders its own settings React."""
    extensions.expect_loaded(LINEAR_SOURCE, name="Linear", version="0.1.0")
    extensions.open_source_settings(LINEAR_SOURCE)
    expect(extensions.get_source_row(LINEAR_SOURCE)).to_contain_text(LINEAR_SETTINGS_TEXT)


def test_bundled_linear_extension_loads_and_renders(
    sculptor_instance_factory_: SculptorInstanceFactory,
) -> None:
    """In a browser, the bundled Linear extension loads and renders its UI."""
    with sculptor_instance_factory_.spawn_instance() as instance:
        settings_page = navigate_to_settings_page(page=instance.page)
        extensions = settings_page.click_on_extensions()
        _assert_linear_extension_loads_and_renders(extensions)


def test_bundled_linear_extension_workspace_widget_shows_branch_ticket(
    sculptor_instance_factory_: SculptorInstanceFactory,
) -> None:
    """The extension's workspace widget renders the branch ticket beside the PR button.
    ...
    extension contributes to the workspace banner resolves the branch's (primary)
    ...
    with sculptor_instance_factory_.spawn_instance() as instance:
        ...
        extensions = settings_page.click_on_extensions()
        extensions.expect_loaded(LINEAR_SOURCE, name="Linear", version="0.1.0")
        _set_linear_api_key(extensions, "lin_api_test")
...
def test_bundled_linear_extension_loads_and_renders_in_electron(
    sculptor_instance_: SculptorInstance,
) -> None:
    """In Electron, the bundled Linear extension loads and renders its UI."""
