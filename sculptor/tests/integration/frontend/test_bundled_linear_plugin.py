...
def test_bundled_linear_plugin_loads_and_renders(
    ...
    Seeded with the experimental flag on, the per-test factory instance runs in
    both launch modes from one config, so a single test covers the plugin's
    load + render in a browser and inside a real, non-packaged Electron shell.
    """
