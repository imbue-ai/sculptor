...
The flow lives behind the experimental ``enable_frontend_plugins`` flag. Each
test uses a fresh factory instance seeded with the flag on via a folder
populator and runs in both browser and Electron launch modes
(``@browser_and_electron``) from that one seeded config -- the per-test factory
can launch a real, non-packaged Electron shell, so no runtime flag toggle is
needed. The same loader assertions back all of them, shared through
``_exercise_*`` helpers.
