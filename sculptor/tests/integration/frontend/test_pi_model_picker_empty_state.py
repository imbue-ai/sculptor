...
def test_pi_picker_empty_state_shows_login_cta(
    ...
    with sculptor_instance_factory_.spawn_instance() as instance:
        ...
        empty_state = page.get_by_test_id(ElementIDs.PI_PICKER_EMPTY_STATE)
        expect(empty_state).to_be_visible(timeout=30_000)
        expect(empty_state).to_contain_text(_NO_MODELS_COPY)
        expect(page.get_by_test_id(ElementIDs.MODEL_SELECTOR)).to_have_count(0)

        # The CTA routes to Settings -> Pi.
        page.get_by_test_id(ElementIDs.PI_PICKER_LOGIN_CTA).click()
