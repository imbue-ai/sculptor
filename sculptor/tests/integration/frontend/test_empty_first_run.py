"""Integration tests for the empty first-run page (FIRST-01..05).
...
the WSC-05 new-workspace form inline in a card. The first prompt defaults to the
``/sculptor:help`` action text (FIRST-04); the sidebar shows the empty-state
"Add a repo" / "No workspaces yet" affordances (FIRST-02); Cmd+K and the global
shortcuts are off so only the form + Settings are reachable (FIRST-03); and
creating the first workspace navigates to the full workspace page (FIRST-05).
...
def test_empty_state_shows_inline_form(sculptor_instance_empty_first_run_: SculptorInstance) -> None:
    """With zero workspaces the empty first-run page renders the inline form (FIRST-01)."""
...
def test_first_prompt_is_prefilled_with_help(sculptor_instance_empty_first_run_: SculptorInstance) -> None:
    """The inline form's prompt is prefilled with the /sculptor:help text (FIRST-04)."""
    page = sculptor_instance_empty_first_run_.page
    empty = PlaywrightEmptyFirstRun(page)
...
def test_sidebar_shows_no_workspaces_hint(sculptor_instance_empty_first_run_: SculptorInstance) -> None:
    """With a repo registered but no workspaces, the sidebar shows the hint (FIRST-02)."""
    page = sculptor_instance_empty_first_run_.page
    empty = PlaywrightEmptyFirstRun(page)
...
def test_sidebar_shows_add_repo_when_no_projects(sculptor_instance_empty_first_run_: SculptorInstance) -> None:
    """With zero repos, the sidebar shows the "Add a repo" button (FIRST-02).
...
def test_global_shortcuts_disabled_in_empty_state(sculptor_instance_empty_first_run_: SculptorInstance) -> None:
    """Cmd+K and the new-workspace shortcut are off in the empty state (FIRST-03).
...
def test_creating_first_workspace_navigates_to_workspace(
    ...
    """Creating the first workspace flips the gate off and shows the workspace page (FIRST-05)."""
    page = sculptor_instance_empty_first_run_.page
    empty = PlaywrightEmptyFirstRun(page)
