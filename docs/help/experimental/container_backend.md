...
The [container recipe](https://github.com/imbue-ai/sculptor/tree/main/container/recipes/docker) (`container/recipes/docker/` in this repo) provides a working Docker setup that:
...
```bash
git clone --depth 1 https://github.com/imbue-ai/sculptor.git
cp -r sculptor/container/recipes/docker sculptor-docker
cd sculptor-docker
```
...
The recipe contains configuration options (environment variables for ports, volumes, binary sources, etc.) and troubleshooting tips. You can read through its README and scripts directly, or point an agent at the copied recipe to explore and customize it for your setup.
...
We encourage you to experiment with using an agent to iterate on the custom backend command to best suit your needs. The container recipe serves as a useful reference for how to download the backend binaries and what the backend process expects (bind address, port, signal handling, etc.).
...
> **Note:** The custom backend command is an unstable feature and the backend interface is subject to change. The recipe and this guide will be updated as the interface evolves.
