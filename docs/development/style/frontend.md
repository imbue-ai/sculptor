...
Many of our conventions and guidelines from [our backend style guide](./backend.md) will carry over here, and I'd recommend reading that document alongside this one.

As much as possible, we've encoded these rules into our eslint configuration ([eslint.config.ts](../../../sculptor/frontend/eslint.config.ts)). Note, there are some minor details not included here but enforced via eslint rules, the file itself should be well documented.

For details about our current tech stack and why were made certain choices, see the [following document](../frontend_details.md).
...
See [our python style guide](./backend.md) for general naming conventions. Below are some additional rules that apply to frontend code.
...
1. **Radix UI Props** (highest priority) — Use component props like `<Flex gap="2">`, `<Text size="2">` when the component supports it.
2. **CSS Variables (Design Tokens)** — Use `var()` references to Radix or custom tokens in SCSS modules.
3. **SCSS Modules** — For layout and component-specific styles not expressible through props.
4. **Inline Styles** (lowest priority, avoid) — Only when you need runtime-computed dynamic values (e.g., values from JS state or calculations).

### Design Tokens

Use design tokens instead of hardcoded values. This is enforced by our custom stylelint plugin.
...
Token sources:
* **Spacing, font sizes, radius, font weights, colors** — Radix UI built-in tokens (e.g., `--space-*`, `--font-size-*`, `--radius-*`, `--gray-*`)
* **Animation timing, z-index, shadows, semantic colors** — Custom tokens in [`src/styles/tokens.css`](../../../sculptor/frontend/src/styles/tokens.css)

Run `npm run lint:styles` to check for violations.

### General Rules

* We use a combination of Radix and SCSS modules for styling our components. See our [tech stack document](../frontend_details.md) for more information on why we landed on this.
...
* Radix overrides live in [`src/styles/radix-overrides.css`](../../../sculptor/frontend/src/styles/radix-overrides.css).
