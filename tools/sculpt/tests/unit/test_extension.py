"""Unit tests for `sculpt extension` helpers: error shaping and renderer labels."""
...
from sculpt.commands.extension import _parse_structured_error
from sculpt.commands.extension import _renderer_label
from sculpt.commands.extension import _results_or_exit
from sculpt.commands.extension import extension_app
from sculpt.formatting import json_error
from sculpt.main import app


def _result_with_renderer(base: None | str = None, base_unset: bool = False) -> ExtensionCommandResult:
    ...
    return ExtensionCommandResult(correlation_id="c", renderer=renderer, op="list", ok=True)


class TestParseStructuredError:
    def test_extracts_code_and_message(self) -> None:
        body = json.dumps(
            {"detail": {"code": "agent_extension_loading_disabled", "message": "Agent extension loading is disabled."}}
        ).encode()
        assert _parse_structured_error(body) == (
            "agent_extension_loading_disabled",
            "Agent extension loading is disabled.",
        )
...
class TestResultsOrExit:
    def test_empty_results_error_carries_the_condition_code(self, capsys: pytest.CaptureFixture[str]) -> None:
        response = ExtensionCommandResponse(correlation_id="c", results=[])
...
class TestCommandRegistration:
    def _group_names(self) -> dict[str, typer.models.TyperInfo]:
        return {group.name: group for group in app.registered_groups if group.name is not None}

    def test_extension_group_is_registered_and_visible(self) -> None:
        groups = self._group_names()
        assert "extension" in groups
        assert groups["extension"].typer_instance is extension_app
        assert groups["extension"].hidden is not True

    def test_plugin_is_a_hidden_alias_of_extension(self) -> None:
        groups = self._group_names()
        assert "plugin" in groups
        assert groups["plugin"].typer_instance is extension_app
        assert groups["plugin"].hidden is True
